"""Feature-gated, ikke-eksekverende desktop action preview (Computer Use I4).

Modulet udsteder korttidsplaner for een praecis, screenshot-bundet handling.
Det fanger intet, injicerer intet og registrerer intet uden `KALIV_COMPUTER_USE`.

Historik, saa den ikke gaar tabt: implementeringen laa i
`desktop_action_preview_tool_legacy` med en facade ovenpaa, der rettede EEN
closure-binding-fejl i ToolGate-installeren -- screenshot- og preview-wrapperne
delte een closure-celle, saa den anden tildeling fik screenshot-wrapperen til at
kalde sig selv rekursivt. Facaden og implementeringen er foldet sammen ved
loeftet til main (30/07-2026): rettelsen ER nu implementeringen, og der er kun
eet modul at laese.
"""
...
_ORIGINAL_DESKTOP_DENIED = DesktopDenied
...
class _PreviewDenied(
    ...
    """Een fail-closed preview-afvisning, synlig gennem begge offentlige
    kontrakter."""


# Preview-funktionerne slaar DesktopDenied op i modulets globals paa
# KALDETIDSPUNKTET. Een dual-contract afvisning daekker derfor begge
# kontrakter, mens den oprindelige base bevares, saa afvisninger rejst af
# desktop_action_plan stadig fanges af `except DesktopDenied`.
DesktopDenied = _PreviewDenied
