...
This module executes an already-authorized ``AuthorizedRigGateRead`` without
turning RigGate into a normal runtime capability:

* origin and bearer-file location are deployment injected, never model supplied;
* exact T-038 durable scope is re-authorized immediately before DNS/file/network;
* DNS is pinned to one validated numeric peer and the connected peer must match;
* plaintext HTTP requires explicit opt-in and may never target a global peer;
* exactly one GET is issued, redirects are never followed, response bytes are bounded;
* only the strict RigGate v1 status projection crosses the boundary;
* the already-claimed T-032 receipt is always finished completed/failed/blocked.

There is deliberately no ToolGate/FastAPI registration, wake/control execution,
environment discovery, retry loop, provider-origin discovery or production
    ...
    def request(
        ...
        *,
        ...
        if parsed.query or parsed.fragment:
            raise RigGateTransportError("RigGate request URL must not contain query/fragment")
        if not parsed.path.startswith("/v1/rigs/") or not parsed.path.endswith(("/health", "/power-readiness")):
            raise RigGateTransportError("RigGate request URL is not a v1 status read")
        origin_text = f"{parsed.scheme}://{parsed.netloc}"
        scheme, host, port, canonical = _origin(origin_text)
        if not url.startswith(canonical + "/v1/rigs/"):
            raise RigGateTransportError("RigGate request URL origin drifted")
