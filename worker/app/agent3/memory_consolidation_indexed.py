...
def apply_protected_consolidation_plan_indexed(
    ...
    *,
    ...
    """Apply W02-B with an explicitly migrated protected blind index.

    The index narrows only canonical verbatim exact-match candidates. Every hit is
    decrypted and fingerprint-verified, then the ordinary W02-A planner is rerun
    while the protected writer's BEGIN IMMEDIATE transaction is held. The index
    never grants fact, review, supersede or write authority.

    The landed non-indexed W02-B path remains unchanged for stores that have not
    opted into the migration. This indexed entry point fails closed if its sidecar
    is absent, incomplete or contains stale lifecycle rows.
    """
...
def _indexed_snapshot_locked(
    ...
    _add_structured_touched_rows(reader, plan, records)
    ...
    keys = sorted(
        {
            ...
            if not _is_verbatim(candidate) and candidate.sensitivity != "secret"
        }
    )
    if not keys:
        return
...
def _add_structured_touched_rows(
    reader: ProtectedMemoryReader,
    plan: ConsolidationPlan,
    records: dict[str, MemoryRecord],
) -> None:
    # Canonical verbatim targets must be rediscovered through the blind index.
    # A plan-supplied existing_id is never allowed to bypass #1216 selection.
    touched = sorted(
        {
            action.existing_id
    ...
    if len(rows) > MAX_CONSOLIDATION_EXISTING:
        raise IndexedMemoryConsolidationWriteError(
            "structured touched snapshot exceeds its hard bound"
        )
    for row in rows:
        record = reader._record(row, access=MemoryReadAccess.LOCAL_MANAGEMENT)
        records[record.id] = record
...
def _sync_created_actions_locked(
    writer: ProtectedMemoryWriter,
    lookup: ProtectedMemoryVerbatimLookup,
    plan: ConsolidationPlan,
    ...
    for action in plan.actions:
        ...
        if action.decision == "supersede":
            if not action.existing_id:
                raise IndexedMemoryConsolidationWriteError(
                    "W02-B supersede action lost its predecessor id"
                )
            lookup.remove_locked(writer._conn, memory_id=action.existing_id)
