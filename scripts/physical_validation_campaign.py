...
def _load_sibling(module_name: str):
    """Load a sibling script by path, not by ``sys.path`` luck.

    Four existing gates (candidate_campaign, candidate_gate,
    stage_a_physical_operator, staged_promotion_runbook) load THIS file with
    ``spec_from_file_location`` and never put ``scripts/`` on ``sys.path``, so a
    bare ``import physical_validation_campaign_core`` raised
    ModuleNotFoundError in all four. Same pattern as ``_load_frozen_attestation``
    in the core. The module is registered under its canonical name so callers
    that DO import it normally get the same object.
    """
    import importlib.util
    import sys as _s

    existing = _s.modules.get(module_name)
    ...
    _s.modules[module_name] = module
    spec.loader.exec_module(module)
    return module


_core = _load_sibling("physical_validation_campaign_core")
_task_ui = _load_sibling("physical_validation_task_ui")
