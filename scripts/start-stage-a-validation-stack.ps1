...
param(
    ...
    [ValidateNotNullOrEmpty()]
    [string]$BackendHost = $(if ($env:MODELRIG_HOST) { $env:MODELRIG_HOST } else { "127.0.0.1" }),
...
# overstyres med -BackendHost eller MODELRIG_HOST, og den valgte adresse
# skrives ud, saa en uNaaelig backend fejler synligt i stedet for at se ud som
# om appen "holdt op med at virke".
Write-Host "  Backend bindes til $BackendHost`:8080" -ForegroundColor Cyan
if ($BackendHost -eq "127.0.0.1") {
    Write-Host "  BEMAERK: telefonen kan ikke naa en loopback-bundet backend. Brug -BackendHost 0.0.0.0 eller saet MODELRIG_HOST=0.0.0.0 foer start hvis enheden skal deltage." -ForegroundColor Yellow
}
