#!/usr/bin/env python3
"""One exact current-main candidate for the remaining Milestone 3 physical work.

The coordinator reuses the existing T-020, T-022 and T-023 operators. Each child
runs in a separate Python process and is rebound to one exact branch/version
before ``main()``. A zero child exit code is never trusted without reopening the
canonical report and proving schema, exact candidate SHA and non-activation.

This coordinator cannot approve a write, observe a device, merge, push, tag,
...
OPERATORS = (
    ...
    (
        ...
        SCRIPTS / "agent3_write_pilot_current_main.py",
        ROOT / "validation" / "agent3-write-pilot-final-gate-latest.json",
...
class Milestone3Error(RuntimeError):
    """The physical campaign cannot continue safely."""
...
def run_stage_a_gate() -> int:
    ...
    result = subprocess.run(
        [sys.executable, str(STAGE_A_GATE), "--report", str(STAGE_A_REPORT)],
        cwd=ROOT,
        env=os.environ.copy(),
        check=False,
    )
...
def main() -> int:
    os.chdir(ROOT)
    heading("Kaliv Milestone 3 — samlet current-main fysisk kandidat")
    print("  Én kandidat. Tre uafhængigt resumable operatorer.")
    print("  Rækkefølge: T-020 read-only → T-022 final-gate → T-023 termination UI.")
    ...
    if stage_code != 0:
        print(
            f"\n  SIKKERT STOP: Stage A-gaten returnerede {stage_code}; Milestone 3 må ikke starte.",
            file=sys.stderr,
        )
        return stage_code
