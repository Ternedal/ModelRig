...
TIMELINE_SCHEMA = "modelrig-agent4/campaign-timeline-entry/v1"
...
SCHEMA_HASH_FIELDS: dict[str, frozenset[str]] = {
    ...
    TIMELINE_SCHEMA: frozenset({"previous_hash", "entry_hash"}),
...
def load_json(path: Path) -> Any:
    require(
        ...
        f"missing or unsafe JSON evidence: {path}",
    )
    return json.loads(
...
def safe_output_root(path: Path) -> Path:
    ...
    require(
        ...
        "A4-18R output root is missing or unsafe",
    )
    try:
    ...
    require(
        ...
        "A4-18R output root may not be a filesystem root",
    )
    return resolved
...
def trial_name_for_path(path: str) -> str | None:
    marker = ".trials."
    if marker not in path:
        return None
    tail = path.rsplit(marker, 1)[1]
    if "." in tail or "[" in tail or tail not in REQUIRED_TRIALS:
        return None
    return tail


def allowed_structural_key(path: str, name: str) -> bool:
    """Allow only the exact fixed trial names to contain words like 'token'."""

    return path.endswith(".trials") and name in REQUIRED_TRIALS


def is_evidence_reference(parent: Mapping[str, Any]) -> bool:
    return {
        "evidence_id",
        "media_type",
        "location",
...
def canonical_hash_slot(
    ...
    if name in SCHEMA_HASH_FIELDS.get(active_schema or "", frozenset()):
        return True

    trial_name = trial_name_for_path(path)
    if trial_name is not None and name in {"payload_sha256", "cursor_sha256"}:
        return True

    if (
        active_schema == FIXTURE_SCHEMA
        and name == "sha256"
        and ".persisted_files[" in path
        and {"path", "size_bytes", "sha256"}.issubset(parent.keys())
    ):
        return True

    if (
        active_schema in {TIMELINE_SCHEMA, EVIDENCE_RECORD_SCHEMA}
        and name == "sha256"
        and is_evidence_reference(parent)
    ):
        return True
...
def scan_text(value: str, *, label: str) -> None:
    for pattern in TEXT_PATTERNS:
        require(
            ...
            f"credential-like value at {label}",
        )
...
def validate_self_digest(value: Mapping[str, Any], *, label: str) -> None:
    ...
    require(
        isinstance(claimed, str) and SHA256_RE.fullmatch(claimed) is not None,
        f"{label} digest missing",
    )
    body = dict(value)
...
def validate_trials(trials: Any) -> None:
    ...
    for name in REQUIRED_TRIALS:
        ...
        if expected is not None:
            require(
                entry.get("http_status") == expected,
                f"trial {name} has wrong HTTP status",
            )
        route = entry.get("route")
        if route is not None:
            require(
                isinstance(route, str) and route.startswith("/api/"),
                f"trial {name} has unsafe route",
            )
            require(
                "?" not in route and "#" not in route and "://" not in route,
                f"trial {name} route leaks request context",
            )
        if name in PAYLOAD_REQUIRED:
            require(
                isinstance(entry.get("payload_sha256"), str)
                and SHA256_RE.fullmatch(entry["payload_sha256"]) is not None,
                f"trial {name} lacks payload digest",
            )
        if name in CURSOR_REQUIRED:
            require(
                isinstance(entry.get("cursor_sha256"), str)
                and SHA256_RE.fullmatch(entry["cursor_sha256"]) is not None,
                f"trial {name} lacks cursor digest",
            )
        require(
...
def validate_fixture(fixture: Any, expected_sha: str) -> None:
    ...
    require(
        fixture.get("repository_sha") == expected_sha,
        "fixture exact SHA mismatch",
    )
    require(
    ...
    for name in (
        ...
        require(
            isinstance(fixture.get(name), str)
            and SHA256_RE.fullmatch(fixture[name]) is not None,
            f"fixture {name} is not a digest",
        )
    for name in (
        "external_dispatch",
        "background_runtime",
...
def validate_mutations(values: Any, expected_sha: str) -> None:
    ...
    require(
        len(items) == 2,
        "exactly two A4-18R snapshot mutations are required",
    )
    ...
    for item in items:
        ...
        require(
            item.get("repository_sha") == expected_sha,
            "mutation exact SHA mismatch",
        )
        validate_self_digest(item, label="mutation")
        mode = item.get("mode")
        require(
            mode in {"campaign-record", "summary"} and mode not in by_mode,
    ...
    require(
        set(by_mode) == {"campaign-record", "summary"},
        "both mutation modes are required",
    )

    campaign = by_mode["campaign-record"]
    require(
        campaign["campaign_count_after"] == campaign["campaign_count_before"] + 1,
        "campaign mutation did not add exactly one campaign",
    )
    require(
        campaign["evidence_count_after"] == campaign["evidence_count_before"],
        "campaign mutation changed evidence count",
    )
    require(
        campaign["timeline_head_after"] == campaign["timeline_head_before"],
        "campaign mutation changed timeline summary",
    )
    require(
        campaign["evidence_head_after"] == campaign["evidence_head_before"],
        "campaign mutation changed evidence summary",
    )

    summary = by_mode["summary"]
    require(
        summary["campaign_count_after"] == summary["campaign_count_before"],
        "summary mutation changed campaign count",
    )
    require(
        summary["evidence_count_after"] == summary["evidence_count_before"] + 1,
        "summary mutation did not add one evidence record",
    )
    require(
        summary["timeline_head_after"] != summary["timeline_head_before"],
        "summary mutation did not change timeline head",
    )
    require(
        summary["evidence_head_after"] != summary["evidence_head_before"],
...
def validate_artifacts(
    ...
    for entry in artifacts:
        require(
            ...
            "artifact receipt has wrong schema",
        )
...
def scan_output_tree(output_root: Path) -> None:
    for path in sorted(output_root.rglob("*")):
        require(
            not path.is_symlink(),
            f"A4-18R evidence tree may not contain symlink: {path}",
        )
...
def audit_evidence(
    ...
    *,
    ...
    require(
        ...
        "expected SHA must be 40 lowercase hex",
    )
    root = safe_output_root(output_root)
    marker = load_json(root / ".modelrig-a4-18r-output.json")
    require(
        isinstance(marker, Mapping) and marker.get("schema") == MARKER_SCHEMA,
        "A4-18R output marker is invalid",
    )
    require(
        marker.get("repository_sha") == expected_sha,
        "output marker exact SHA mismatch",
    )
    require(
        marker.get("public_network") is False
        and marker.get("production_activation") is False,
        "output marker widened authority",
    )
    require(
        not (root / "admin-key.txt").exists(),
        "ephemeral admin key still exists after cleanup",
    )
    require(
        not (root / "modelrig-data.json").exists(),
        "pairing/device store still exists after cleanup",
    )
    ...
    require(
        receipt.get("human_decision") in {"GO", "NO-GO"},
    ...
    require(
        isinstance(pixel.get("model"), str)
        and str(pixel["model"]).startswith("Pixel"),
        "physical device model is not Pixel",
    )
    require(
        isinstance(pixel.get("serial_sha256"), str)
        and SHA256_RE.fullmatch(pixel["serial_sha256"]) is not None,
        "Pixel serial digest is invalid",
    )
    require(
