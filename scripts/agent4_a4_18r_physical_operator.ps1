...
function Resolve-ExternalOutputRoot {
    if ([string]::IsNullOrWhiteSpace($OutputRoot)) {
        throw "A4-18R kræver -OutputRoot uden for repositoryet."
    }
...
function Assert-PrivateLocalLanAddress {
    ...
    if ($null -eq $profile -or [string]$profile.NetworkCategory -ne "Private") {
        throw "A4-18R kræver Windows-netværksprofil Private."
    }
}
...
function Get-AdbValue {
    param([string]$DeviceSerial, [string]$Property)
...
function Resolve-PhysicalPixel {
    ...
    if ($manufacturer -ne "Google" -or $model -notmatch "^Pixel\b") {
        throw "A4-18R kræver en fysisk Google Pixel; fandt '$manufacturer $model'."
    }
...
function Resolve-PixelPrivateIp {
    param([string]$DeviceSerial, [string]$TargetAddress)
...
function Read-State {
    ...
    if ([string]$state.schema -ne "modelrig-agent4/a4-18r-operator-state/v1") { throw "A4-18R state har forkert schema." }
...
function Read-Observations {
    ...
    $value = Get-Content -LiteralPath $observationsPath -Raw | ConvertFrom-Json
    if ([string]$value.schema -ne "modelrig-agent4/a4-18r-observations/v1" -or [string]$value.repository_sha -ne $ExpectedSha) {
        throw "A4-18R observationsfil har forkert authority."
    }
    return $value
}
...
function Assert-StatePhase {
    ...
    if ($Allowed -notcontains [string]$State.phase) {
        throw "A4-18R handling er ikke tilladt i fase '$($State.phase)'; forventede $($Allowed -join ', ')."
    }
}
...
function Test-ExpectedProcess {
    ...
    return [string]$process.Name -ieq "python.exe" -and
        $command -match "uvicorn\s+app\.entrypoint:app" -and
        $command -match "--host\s+127\.0\.0\.1" -and
        $command -match "--port\s+$workerPort"
}
...
function Stop-RecordedProcess {
    ...
    if (-not (Test-ExpectedProcess -ProcessId $ProcessId -Kind $Kind)) {
        throw "Recorded $Kind PID tilhører ikke A4-18R; processen bevares."
    }
    Stop-Process -Id $ProcessId -Force -ErrorAction Stop
    $deadline = (Get-Date).AddSeconds(20)
    while ($null -ne (Get-ProcessInfo -ProcessId $ProcessId) -and (Get-Date) -lt $deadline) {
...
function Install-A4FirewallRule {
    param([string]$Address, [string]$PixelIp)
...
function Invoke-WithEnvironment {
    param([hashtable]$Values, [scriptblock]$Operation)
    $saved = @{}
...
function Start-Worker {
    param([ValidateSet("off", "enabled")][string]$Mode)
    ...
    Invoke-WithEnvironment -Values @{
        ...
        Start-Process -FilePath $python `
            -ArgumentList @("-u", "-m", "uvicorn", "app.entrypoint:app", "--host", "127.0.0.1", "--port", "$workerPort") `
            -WorkingDirectory (Join-Path $repoRoot "worker") `
            -RedirectStandardOutput (Join-Path $logsDir "worker.stdout.log") `
            -RedirectStandardError (Join-Path $logsDir "worker.stderr.log") | Out-Null
    }
    Wait-Http200 -Url "http://127.0.0.1:$workerPort/healthz"
    $rows = @(Get-ListenerRows -Port $workerPort)
    if ($rows.Count -ne 1 -or [string]$rows[0].LocalAddress -ne "127.0.0.1" -or
        -not (Test-ExpectedProcess -ProcessId ([int]$rows[0].OwningProcess) -Kind worker)) {
...
function Start-Backend {
    ...
    Invoke-WithEnvironment -Values @{
        ...
        Start-Process -FilePath $backendExe -WorkingDirectory $output `
            -RedirectStandardOutput (Join-Path $logsDir "$LogLabel.stdout.log") `
            -RedirectStandardError (Join-Path $logsDir "$LogLabel.stderr.log") | Out-Null
    }
    Wait-Http200 -Url "http://${HostAddress}:$backendPort/healthz"
    $rows = @(Get-ListenerRows -Port $backendPort)
    if ($rows.Count -ne 1 -or [string]$rows[0].LocalAddress -ne $HostAddress -or
        -not (Test-ExpectedProcess -ProcessId ([int]$rows[0].OwningProcess) -Kind backend)) {
...
function Build-PhysicalArtifacts {
    ...
    if (-not [string]::IsNullOrWhiteSpace($ApkPath)) {
        ...
        $apks = @(Get-ChildItem -LiteralPath (Join-Path $repoRoot "android\app\build\outputs\apk\a425f") -Filter "*.apk" -File)
...
function Start-LanStack {
    param($State, [ValidateSet("off", "enabled")][string]$Mode)
    $State.worker_pid = Start-Worker -Mode $Mode
    try {
        $State.backend_pid = Start-Backend -HostAddress ([string]$State.lan_address) `
            -OperatorEnabled:($Mode -eq "enabled") -GrantAdmin:$false -LogLabel "backend-lan"
    } catch {
...
function Invoke-GrantTransition {
    param([ValidateSet("grant", "revoke")][string]$Transition, $State)
    ...
    try {
        ...
        try {
            ...
            if ($Transition -eq "grant") {
                & $grantExe -grant $deviceId -url "http://127.0.0.1:$backendPort"
            } else {
                & $grantExe -revoke $deviceId -url "http://127.0.0.1:$backendPort"
            }
            if ($LASTEXITCODE -ne 0) { throw "A4-18R grant CLI afviste $Transition." }
        } finally {
            [Environment]::SetEnvironmentVariable("MODELRIG_ADMIN_KEY", $saved, "Process")
        }
    } catch {
        $transitionError = $_.Exception
    } finally {
        if ($adminPid -gt 0) { Stop-RecordedProcess -ProcessId $adminPid -Kind backend }
        if (@(Get-ListenerRows -Port $backendPort).Count -eq 0) {
    ...
    if ($null -ne $transitionError) { throw $transitionError }
    $State.device_id = $deviceId
    Write-State -State $State
}

function Invoke-Mutation {
    param([ValidateSet("campaign-record", "summary")][string]$Mode, $State)
    Stop-RecordedProcess -ProcessId ([int]$State.worker_pid) -Kind worker
    $State.worker_pid = 0
    Write-State -State $State
    ...
    $mutationError = $null
    try {
        & python $mutateScript --output-root $output --data-root $fixtureData --fixture-manifest $fixtureManifest `
            --mode $Mode --receipt $receipt --expected-sha $ExpectedSha
...
function Record-Checkpoint {
    ...
    if ([string]::IsNullOrWhiteSpace($Checkpoint) -or [string]::IsNullOrWhiteSpace($Result)) {
        throw "Record kræver -Checkpoint og -Result."
    }
    if (@($checkpointPhases[$Checkpoint]) -notcontains [string]$State.phase) {
    ...
    if ($Checkpoint -eq "stale_campaign_record_422" -and [string]$State.last_mutation -ne "campaign-record") {
        throw "Campaign stale-check kræver MutateCampaignSnapshot først."
    }
    if ($Checkpoint -eq "stale_summary_422" -and [string]$State.last_mutation -ne "summary") {
        throw "Summary stale-check kræver MutateSummarySnapshot først."
    }
    if ($Result -eq "Pass" -and $expectedHttpStatus.ContainsKey($Checkpoint)) {
        $expected = [int]$expectedHttpStatus[$Checkpoint]
        if ($HttpStatus -ne $expected) { throw "Checkpoint $Checkpoint kræver HTTP $expected for Pass." }
    }
    if ($HttpStatus -ge 0 -and ([string]::IsNullOrWhiteSpace($Route) -or $Route -notmatch "^/api/" -or $Route -match "[?#]" -or $Route -match "://")) {
    ...
    foreach ($pair in @(@($PayloadSha256, "PayloadSha256"), @($CursorSha256, "CursorSha256"))) {
        if (-not [string]::IsNullOrWhiteSpace([string]$pair[0]) -and [string]$pair[0] -notmatch "^sha256:[0-9a-f]{64}$") {
            throw "$($pair[1]) skal være sha256:<64 lowercase hex>."
        }
    }
    if (-not [string]::IsNullOrWhiteSpace($Note) -and
        $Note -match "(?i)authorization\s*:|bearer\s+|pairing[_ -]?code\s*[:=]|device[_ -]?token\s*[:=]|admin[_ -]?key\s*[:=]|\b[A-Z0-9]{4}-[A-Z0-9]{4}\b|\b[0-9a-f]{64}\b") {
    ...
    $entry.http_status = if ($HttpStatus -ge 0) { $HttpStatus } else { $null }
...
function Get-RelativeChildPath {
    ...
    if (-not $full.StartsWith($rootFull, [StringComparison]::OrdinalIgnoreCase)) {
        throw "Artifact ligger uden for sin authority-root: $full"
    }
    return $full.Substring($rootFull.Length).Replace('\', '/')
}

function Get-FileReceipt {
    param([ValidateSet("output", "repository")][string]$Scope, [string]$Path)
...
function Get-ArtifactReceipts {
    ...
    $repoPaths = @(
        ...
        "scripts\agent4_a4_18r_physical_operator.ps1",
        "scripts\agent4_a4_18r_physical_fixture.py",
        "scripts\agent4_a4_18r_physical_mutate.py",
        "scripts\agent4_a4_18r_audit.py",
        "scripts\agent4_a4_18r_fault_host.py",
        "scripts\agent4_a4_18r_fault_window.ps1"
...
function Test-AppInstalled {
    ...
    $ErrorActionPreference = "Continue"
    try {
        $lines = @(& adb -s $DeviceSerial shell pm path $packageName 2>&1 | ForEach-Object { [string]$_ })
        $code = $LASTEXITCODE
    } finally { $ErrorActionPreference = $saved }
    if ($code -ne 0) { throw "Kunne ikke verificere A4-18R app-installationstilstand." }
    return @($lines | Where-Object { $_ -match "^package:" }).Count -gt 0
}

function Stop-Harness {
    param($State, [switch]$Uninstall)
    $recordedBackendPid = [int]$State.backend_pid
    $recordedWorkerPid = [int]$State.worker_pid
    $unknown = $false
    try {
        Stop-RecordedProcess -ProcessId $recordedBackendPid -Kind backend
        Stop-RecordedProcess -ProcessId $recordedWorkerPid -Kind worker
    } catch {
        $unknown = $true
        throw
    }
    $backendStopped = $recordedBackendPid -le 0 -or $null -eq (Get-ProcessInfo -ProcessId $recordedBackendPid)
    $workerStopped = $recordedWorkerPid -le 0 -or $null -eq (Get-ProcessInfo -ProcessId $recordedWorkerPid)
    if (-not $backendStopped -or -not $workerStopped) { throw "A4-18R cleanup kunne ikke bevise de oprindelige recorded PIDs stoppet." }

    Remove-A4FirewallRule
    $packageRemoved = $false
    if ($Uninstall) {
        Invoke-Adb -DeviceSerial ([string]$State.adb_serial) uninstall $packageName | Out-Null
        $packageRemoved = -not (Test-AppInstalled -DeviceSerial ([string]$State.adb_serial))
        if (-not $packageRemoved) { throw "A4-18R testpakken er stadig installeret efter uninstall." }
    }
    Remove-Item -LiteralPath $adminKeyFile -Force -ErrorAction SilentlyContinue
    ...
    $cleanup = [pscustomobject][ordered]@{
        ...
        test_package_uninstalled = if ($Uninstall) { $packageRemoved } else { $false }
...
function Assert-ReplaceWorkspaceAuthority {
    if (-not (Test-Path -LiteralPath $statePath -PathType Leaf) -or
        -not (Test-Path -LiteralPath $markerPath -PathType Leaf)) {
        throw "Refuserer -Replace: eksisterende output er ikke et verificerbart A4-18R workspace."
    }
    ...
    if ([string]$oldState.schema -ne "modelrig-agent4/a4-18r-operator-state/v1" -or
        [string]$oldMarker.schema -ne "modelrig-agent4/a4-18r-output/v1") {
        throw "Refuserer -Replace: eksisterende output er ikke et verificerbart A4-18R workspace."
    }
    if ([string]$oldState.expected_sha -ne $ExpectedSha -or [string]$oldMarker.repository_sha -ne $ExpectedSha) {
        throw "Refuserer -Replace: eksisterende A4-18R workspace tilhører en anden exact SHA."
    }
    if ([string]$oldState.output_root -ne $output) {
        throw "Refuserer -Replace: eksisterende A4-18R workspace peger på en anden output-root."
    }
    return $oldState
}
...
function Write-FinalReceipt {
    ...
    foreach ($name in @(
        "backend_stopped", "worker_stopped", "firewall_removed", "ports_free",
        "credential_file_deleted", "pairing_store_deleted", "test_package_uninstalled"
    )) {
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0 -or $digest -notmatch "^sha256:[0-9a-f]{64}$") {
            throw "Kunne ikke beregne canonical receipt-digest."
        }
...
switch ($Action) {
    "PrepareOff" {
        ...
        if (Test-Path -LiteralPath $output) {
            ...
            $oldState = Assert-ReplaceWorkspaceAuthority
            [void](Stop-Harness -State $oldState -Uninstall)
        ...
        [ordered]@{
            schema = "modelrig-agent4/a4-18r-output/v1"
            repository_sha = $ExpectedSha
            public_network = $false
            production_activation = $false
        ...
        Invoke-Adb -DeviceSerial ([string]$pixel.serial) install -r $apk | Out-Null
        ...
        $state = [pscustomobject][ordered]@{
            ...
            lan_url = "http://${LanAddress}:$backendPort"
            pixel_ip = $pixelIp
    ...
    "Regrant" {
        ...
        Invoke-GrantTransition -Transition grant -State $state
        $state.phase = "regranted"
        Write-State -State $state
    }
    "RestartWorker" {
        $state = Read-State
        Assert-StatePhase -State $state -Allowed @("granted", "regranted")
        Stop-RecordedProcess -ProcessId ([int]$state.worker_pid) -Kind worker
        $state.worker_pid = Start-Worker -Mode enabled
        Write-State -State $state
    }
    "RestartBackend" {
        $state = Read-State
        Assert-StatePhase -State $state -Allowed @("granted", "revoked", "regranted")
        Stop-RecordedProcess -ProcessId ([int]$state.backend_pid) -Kind backend
        $state.backend_pid = Start-Backend -HostAddress ([string]$state.lan_address) -OperatorEnabled:$true -GrantAdmin:$false -LogLabel "backend-lan"
        Write-State -State $state
    }
    ...
    "Status" {
        ...
        [ordered]@{
            ...
            package_name = [string]$state.package_name
    ...
    "Finalize" {
        $state = Read-State
        Write-FinalReceipt -State $state
    }
    "Stop" {
        $state = Read-State
        Stop-Harness -State $state -Uninstall | ConvertTo-Json -Depth 10
    }
}
