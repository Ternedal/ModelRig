...
def json_equal(left: Any, right: Any) -> bool:
    """Compare values with JSON/JSON-Schema type semantics.

    Python considers ``True == 1`` and ``False == 0``. JSON Schema does not:
    booleans are a distinct JSON type. Keep number equality compatible with JSON
    Schema, where 1 and 1.0 represent the same mathematical number, while making
    nested objects/arrays equally strict about boolean-vs-number identity.
    """
...
def validate(schema: Mapping[str, Any], value: Any, *, root: Mapping[str, Any], path: str = "$") -> None:
    ...
    if "enum" in schema:
        enum = schema["enum"]
        require(
            isinstance(enum, list) and any(json_equal(value, candidate) for candidate in enum),
...
def main() -> int:
    ...
    require(passes(cue, valid_cue), "valid semantic BodyCue was rejected")
    require(
        not passes(cue, {"type": "modelrig-body-cue", "version": 1, "utterance_id": "u-01"}),
        "empty BodyCue unexpectedly passed",
    )
    for field, value in (("body_id", "anders-body"), ("intensity", 0.0), ("duration_ms", 0)):
        single_field_cue = {
            "type": "modelrig-body-cue",
            "version": 1,
            "utterance_id": f"u-{field}",
            field: value,
        }
        require(passes(cue, single_field_cue), f"BodyRig-valid {field}-only cue was rejected")
    bool_version_cue = dict(valid_cue)
    bool_version_cue["version"] = True
    require(not passes(cue, bool_version_cue), "BodyCue accepted boolean as numeric version 1")
    ...
    require(
        not passes(bodyprint, {"format": "modelrig-bodyprint", "version": True, "shape": {"energy": 0.5}}),
        "bodyprint accepted boolean as numeric version 1",
    )
    ...
    bool_manifest = dict(valid_manifest)
    bool_manifest["format_version"] = True
    require(not passes(manifest, bool_manifest), "manifest accepted boolean as format_version 1")
    ...
    require("manifest.json" not in checksums.get("required", []), "checksum schema must not require manifest.json")
    require(checksums.get("minProperties") == 4, "checksum schema minimum must match four required payload hashes")
    require(checksums.get("maxProperties") == 10, "checksum schema maximum must match four required + six motion hashes")
    ...
    with_manifest = dict(valid_checksums)
    with_manifest["manifest.json"] = digest
    require(not passes(checksums, with_manifest), "checksum map accepted manifest.json contrary to canonical package format")
    ...
    numeric_synthetic = json.loads(json.dumps(valid_provenance))
    numeric_synthetic["synthetic_avatar"] = 1
    require(not passes(provenance, numeric_synthetic), "provenance accepted numeric 1 as synthetic-avatar boolean")
    bool_provenance_version = json.loads(json.dumps(valid_provenance))
    bool_provenance_version["version"] = True
    require(not passes(provenance, bool_provenance_version), "provenance accepted boolean as numeric version 1")
    ...
    require(
        "other than `manifest.json` and `checksums.json` itself" in spec,
        "MRBODY spec lost canonical checksum exclusion rule",
    )
