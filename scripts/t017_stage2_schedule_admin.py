#!/usr/bin/env python3
"""Apply only T-017 ScheduleAdmin preview, persistence and display changes.

Temporary transport. The diagnostics workflow removes this file before
committing the proven canonical tree.
"""
...
replace_once(PATH, "_APPROVAL_VERSION = 1", "_APPROVAL_VERSION = 2")
...
# Renewing and explicitly re-enabling must keep the stored interpretation.
replace_once(
    PATH,
...
# The renew() path has its own immutable-action preview inside one open store.
replace_once(
    PATH,
    '''                tool=current.tool,\n                args=current.args,\n                cadence=current.cadence,\n                ttl_days=ttl_days,\n                max_runs=max_runs,\n                enable=enable,\n''',
    '''                tool=current.tool,\n                args=current.args,\n                cadence=current.cadence,\n                timezone_name=current.timezone,\n                misfire_policy=current.misfire_policy,\n                ttl_days=ttl_days,\n                max_runs=max_runs,\n                enable=enable,\n''',
...
print("T-017 stage 2 ScheduleAdmin applied")
