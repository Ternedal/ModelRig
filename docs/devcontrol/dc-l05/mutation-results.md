...
6. Capture output without finite byte limits or leave descendants alive after
   timeout: bounded-subprocess contracts fail.
...
10. Activate `windows_catalog_tier_a_contract.py` or
...
The final candidate must run the native contracts on a real Windows kernel and all
portable repository workflows on the same exact head.
