...
| Ignored artifacts count as mutations | Remove `--ignored` or replace `git clean -fdx` with `git clean -fd` | **RED** — ignored-artifact boundary assertion detects the weakened evidence/reset |
...
escaped-session termination and ignored-artifact mutation handling.
