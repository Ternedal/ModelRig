...
fun ScheduleScreen(store: TokenStore, onClose: () -> Unit) {
    ...
    fun previewCreate() {
        ...
        execute(
            action = { client().preview(selectedTool.name, args, cadence.trim(), ttl, runs) },
            success = {
                preview = it
    ...
    Surface(color = KalivTheme.colors.background, modifier = Modifier.fillMaxSize()) {
        Column(
            ...
            ScheduleCard {
                ...
                Text(
                    "Vælg kun blandt værktøjer, som riggens autoritative kontrakt tillader uden opsyn.",
                    color = KalivTheme.colors.textMuted,
                    fontSize = 11.sp,
                )
                ...
                Button(
                    enabled = !busy && selectedTool?.selectable == true && cadence.isNotBlank() && ttlDays.isNotBlank() && maxRuns.isNotBlank(),
