...
MODULE_PATH = ROOT / "devcontrol/src/kaliv_dev_control/_tier_a_materialization.py"
...
class TierAMaterializationExtractionTests(unittest.TestCase):
    def test_module_owns_exact_three_mutable_service_classes(self) -> None:
        ...
        module = importlib.import_module("kaliv_dev_control._tier_a_materialization")
        for name in EXPECTED:
    ...
    def test_core_physically_removes_and_directly_reexports_classes(self) -> None:
        self.assertEqual(_direct_imports(CORE_PATH, "_tier_a_materialization"), EXPECTED)
    ...
    def test_dc_l06_identity_chains_remain_exact(self) -> None:
        ...
        self.assertIs(core._LeaseCapturingVerifier, module._LeaseCapturingVerifier)
        for name in ("LeasedCommandRegistry", "LeasedCatalogMaterializer"):
    ...
    def test_constructor_guards_remain_fail_closed(self) -> None:
        module = importlib.import_module("kaliv_dev_control._tier_a_materialization")
        error = importlib.import_module("kaliv_dev_control._tier_a_lease").TierAExecutionError
        with self.assertRaisesRegex(error, "requires WindowsPhysicalIsolationVerifier"):
            module._LeaseCapturingVerifier(object())
        with self.assertRaisesRegex(error, "requires a command registry"):
    ...
    def test_bundle_contains_module_once_before_importing_core(self) -> None:
        authority = importlib.import_module("kaliv_dev_control.tier_a_authority")
        module_path = "devcontrol/src/kaliv_dev_control/_tier_a_materialization.py"
        core_path = "devcontrol/src/kaliv_dev_control/_tier_a_execution_core.py"
        ...
        tree = ast.parse(AUTHORITY_PATH.read_text(encoding="utf-8"), filename=str(AUTHORITY_PATH))
        literal = None
        for node in tree.body:
