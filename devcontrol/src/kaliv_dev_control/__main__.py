"""Minimal administrative CLI for contracts and operator-owned evidence."""
...
def _attestation(path: str) -> IsolationAttestation:
    ...
    return IsolationAttestation.from_mapping(value)
...
def main() -> int:
    ...
    try:
        if args.command == "sign-physical-report":
            report = load_unsigned_report(Path(args.unsigned_report).resolve())
            secret = load_signing_secret(Path(args.key_file).resolve())
            signed = HmacIsolationReportSigner(args.key_id, secret).sign(report)
            digest = write_signed_report(Path(args.output).resolve(), signed)
            print(digest)
            return 0
