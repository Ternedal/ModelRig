"""Static support values for the DC-L12 publisher evidence chain.
...
from ..publisher_dry_run import PublisherRequestVerifier, SignedPublisherRequest
...
PUBLISHER_AUTHORIZATION_POLICY = (
    "Accept only one complete signed publisher request that still verifies against the exact task, trusted publisher, trusted semantic reviewer and current execution authority.",
...
class PublisherAuthorizationError(ValueError):
    """Publisher authorization, replay or recovery evidence is invalid."""
...
def _load_canonical(
    ...
    *,
    ...
    if (
        ...
        or _has_linkish_component(source.parent)
    ):
        raise PublisherAuthorizationError(f"{name} path is unsafe")
    ...
    if canonical_json is None or canonical_json().encode("utf-8") != payload:
        raise PublisherAuthorizationError(f"{name} is not canonical")
    return value
...
class RemoteRepositoryIdentity:
    ...
    def __post_init__(self) -> None:
        if (
            self.schema != _REMOTE_REPOSITORY_SCHEMA
            or self.provider != "github"
            or self.host != "github.com"
        ):
            raise PublisherAuthorizationError(
                "remote repository provider is unsupported"
            )
        if (
...
class PublisherCredentialPolicy:
    ...
    def __post_init__(self) -> None:
        if self.schema != _CREDENTIAL_POLICY_SCHEMA:
            raise PublisherAuthorizationError(
                "unsupported publisher credential policy schema"
            )
        ...
        if (
            ...
            raise PublisherAuthorizationError(
                "credential policy repository is invalid"
            )
        _clean_text(
            self.head_branch,
            name="credential policy branch",
            maximum=200,
            one_line=True,
        )
        if (
            self.allowed_permissions != _ALLOWED_PERMISSIONS
            or self.denied_capabilities != _DENIED_CAPABILITIES
        ):
            raise PublisherAuthorizationError(
                "credential policy is not least privilege"
            )
        if self.rules_sha256 != publisher_credential_policy_rules_sha256():
            raise PublisherAuthorizationError(
                "credential policy rules hash is unsupported"
            )
        if (
            self.token_material_present
            or self.reusable_credential_allowed
            or not self.branch_scope_exact
            or not self.draft_pr_only
            or self.maximum_uses != 1
        ):
            raise PublisherAuthorizationError(
                "credential policy authority boundary is invalid"
            )
    ...
    def for_request(
        ...
        *,
        ...
        if (
            not isinstance(signed_request, SignedPublisherRequest)
            or not isinstance(remote_repository, RemoteRepositoryIdentity)
        ):
            raise PublisherAuthorizationError("credential policy inputs are invalid")
    ...
    def from_mapping(cls, value: Any) -> "PublisherCredentialPolicy":
        ...
        data = dict(
            _strict(value, name="publisher credential policy", fields=fields)
        )
        if not isinstance(data["allowed_permissions"], list) or not isinstance(
            data["denied_capabilities"], list
        ):
            raise PublisherAuthorizationError(
                "credential policy capability sets must be arrays"
            )
        data["allowed_permissions"] = tuple(data["allowed_permissions"])
        data["denied_capabilities"] = tuple(data["denied_capabilities"])
        return cls(**data)
...
class PublisherAuthorizationLease:
    """Unavailable v1 marker; the supported lease is Ed25519 v2."""


class PublisherAuthorizationVerifier:
    """Read-only interface marker containing no key material."""
...
class PublisherReplayLedgerEntry:
    ...
    one_time: bool = True
    maximum_uses: int = 1
    schema: str = "unsupported-v1-base"
