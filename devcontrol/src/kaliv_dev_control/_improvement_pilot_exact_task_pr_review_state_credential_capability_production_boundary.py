"""Host-pinned production boundary for ADR-DC-078 review-state read capability."""
...
def _canonical_policy_path() -> Path:
    if os.name == "posix":
        return Path(
            ...
            "rsi-pilot-exact-task-pr-review-state-credential-broker-policy-v1.json"
        )
    if os.name == "nt":
        return Path(
            r"C:\Program Files\ModelRig\DevControl\review\rsi-pilot-exact-task-pr-review-state-credential-broker-policy-v1.json"
        )
...
def _canonical_broker_path() -> Path:
    ...
    if os.name == "nt":
        return Path(
            r"C:\Program Files\ModelRig\DevControl\bin\rsi-github-review-state-read-broker-v1.exe"
        )
    raise PilotExactTaskPrReviewStateCredentialCapabilityProductionBoundaryError(
...
def install_pilot_exact_task_pr_review_state_credential_capability_production_boundary(
    implementation: Any,
) -> None:
    ...
    marker = (
        "_production_pilot_exact_task_pr_review_state_credential_capability_boundary_installed"
    )
    if getattr(implementation, marker, False):
        return
    private_materialize = (
        implementation._materialize_verified_pilot_exact_task_pr_review_state_credential_capability
    )

    def materialize_pilot_exact_task_pr_review_state_credential_capability(
        review_observation_checkpoint: Any,
    ) -> Any:
        try:
            descriptor = _canonical_broker_descriptor(implementation)
            return private_materialize(
                review_observation_checkpoint=review_observation_checkpoint,
    ...
    implementation.materialize_pilot_exact_task_pr_review_state_credential_capability = (
