"""Host-pinned production boundary for ADR-DC-084 branch-bound review-thread capability."""
...
PILOT_EXACT_TASK_PR_BRANCH_BOUND_REVIEW_THREAD_CREDENTIAL_BROKER_POLICY_SCHEMA = (
    "kaliv-rsi-pilot-exact-task-pr-branch-bound-review-thread-credential-broker-policy/v1"
)
...
class PilotExactTaskPrBranchBoundReviewThreadCredentialCapabilityProductionBoundaryError(ValueError):
    """Production branch-bound review-thread broker is unsafe or unavailable."""
...
def _canonical_policy_path() -> Path:
    if os.name == "posix":
        return Path(
            ...
            "rsi-pilot-exact-task-pr-branch-bound-review-thread-credential-broker-policy-v1.json"
        )
    if os.name == "nt":
        return Path(
            r"C:\Program Files\ModelRig\DevControl\review"
            + r"\rsi-pilot-exact-task-pr-branch-bound-review-thread-credential-broker-policy-v1.json"
        )
    raise PilotExactTaskPrBranchBoundReviewThreadCredentialCapabilityProductionBoundaryError(
        "branch-bound review-thread broker policy platform is unsupported"
    )


def _canonical_broker_path() -> Path:
    if os.name == "posix":
        return Path("/usr/local/libexec/modelrig/rsi-github-branch-bound-review-thread-read-broker-v1")
    if os.name == "nt":
        return Path(
            r"C:\Program Files\ModelRig\DevControl\bin"
            + r"\rsi-github-branch-bound-review-thread-read-broker-v1.exe"
        )
    raise PilotExactTaskPrBranchBoundReviewThreadCredentialCapabilityProductionBoundaryError(
        "branch-bound review-thread broker executable platform is unsupported"
    )
...
def _canonical_bytes(value: Mapping[str, Any]) -> bytes:
    try:
        ...
        raise PilotExactTaskPrBranchBoundReviewThreadCredentialCapabilityProductionBoundaryError(
            "branch-bound review-thread broker policy is not canonical JSON"
        ) from exc
...
def _parse_policy(payload: bytes, *, broker_path: Path, implementation: Any) -> Mapping[str, Any]:
    if not isinstance(payload, bytes) or not payload:
        raise PilotExactTaskPrBranchBoundReviewThreadCredentialCapabilityProductionBoundaryError(
            "branch-bound review-thread broker policy payload is missing"
        )
    try:
        value = json.loads(payload.decode("utf-8", errors="strict"))
    except (UnicodeError, json.JSONDecodeError) as exc:
        raise PilotExactTaskPrBranchBoundReviewThreadCredentialCapabilityProductionBoundaryError(
            "branch-bound review-thread broker policy is invalid JSON"
        ) from exc
    if not isinstance(value, Mapping) or set(value) != _POLICY_FIELDS:
        raise PilotExactTaskPrBranchBoundReviewThreadCredentialCapabilityProductionBoundaryError(
            "branch-bound review-thread broker policy fields mismatch"
        )
    expected = {
        "schema": PILOT_EXACT_TASK_PR_BRANCH_BOUND_REVIEW_THREAD_CREDENTIAL_BROKER_POLICY_SCHEMA,
        "provider": "github",
        "graphql_endpoint": implementation.PILOT_EXACT_TASK_PR_BRANCH_BOUND_REVIEW_THREAD_GRAPHQL_ENDPOINT,
        "repository": "Ternedal/ModelRig",
        "credential_account": implementation.PILOT_EXACT_TASK_PR_BRANCH_BOUND_REVIEW_THREAD_CREDENTIAL_ACCOUNT,
        "credential_protocol": implementation.PILOT_EXACT_TASK_PR_BRANCH_BOUND_REVIEW_THREAD_CREDENTIAL_PROTOCOL,
        "operation": implementation.PILOT_EXACT_TASK_PR_BRANCH_BOUND_REVIEW_THREAD_GRAPHQL_OPERATION,
        "broker_executable_path": os.fspath(broker_path),
        "secret_source": implementation.PILOT_EXACT_TASK_PR_BRANCH_BOUND_REVIEW_THREAD_SECRET_SOURCE,
        "secret_transport": implementation.PILOT_EXACT_TASK_PR_BRANCH_BOUND_REVIEW_THREAD_SECRET_TRANSPORT,
        "graphql_query_sha256": hashlib.sha256(
            implementation.PILOT_EXACT_TASK_PR_BRANCH_BOUND_REVIEW_THREAD_GRAPHQL_QUERY.encode("utf-8")
        ).hexdigest(),
    ...
    if mismatch is not None:
        raise PilotExactTaskPrBranchBoundReviewThreadCredentialCapabilityProductionBoundaryError(
            f"branch-bound review-thread broker policy mismatch: {mismatch}"
        )
    ...
    if (
        ...
        raise PilotExactTaskPrBranchBoundReviewThreadCredentialCapabilityProductionBoundaryError(
            "branch-bound review-thread broker policy version/hash is invalid"
        )
    if payload != _canonical_bytes(dict(value)):
        raise PilotExactTaskPrBranchBoundReviewThreadCredentialCapabilityProductionBoundaryError(
            "branch-bound review-thread broker policy is not byte-for-byte canonical"
        )
    return dict(value)
...
def _read_broker_bytes(path: Path, *, require_host_control: bool) -> bytes:
    ...
    if not candidate.is_absolute() or _has_linkish_component(candidate):
        raise PilotExactTaskPrBranchBoundReviewThreadCredentialCapabilityProductionBoundaryError(
            "branch-bound review-thread broker executable path is unsafe"
        )
    ...
    try:
        ...
        raise PilotExactTaskPrBranchBoundReviewThreadCredentialCapabilityProductionBoundaryError(
            "branch-bound review-thread broker executable is missing or unreadable"
        ) from exc
    try:
        ...
        if (
            ...
            raise PilotExactTaskPrBranchBoundReviewThreadCredentialCapabilityProductionBoundaryError(
                "branch-bound review-thread broker executable file is unsafe"
            )
        if require_host_control:
            try:
                ...
                raise PilotExactTaskPrBranchBoundReviewThreadCredentialCapabilityProductionBoundaryError(
                    "branch-bound review-thread broker executable is not host-admin controlled"
                ) from exc
        ...
        while remaining:
            ...
            if not chunk:
                raise PilotExactTaskPrBranchBoundReviewThreadCredentialCapabilityProductionBoundaryError(
                    "branch-bound review-thread broker executable read was incomplete"
                )
            chunks.append(chunk)
            remaining -= len(chunk)
        if os.read(descriptor, 1):
            raise PilotExactTaskPrBranchBoundReviewThreadCredentialCapabilityProductionBoundaryError(
                "branch-bound review-thread broker executable changed while reading"
            )
        after = os.fstat(descriptor)
        if (observed.st_dev, observed.st_ino, observed.st_size) != (after.st_dev, after.st_ino, after.st_size):
            raise PilotExactTaskPrBranchBoundReviewThreadCredentialCapabilityProductionBoundaryError(
                "branch-bound review-thread broker executable identity changed during read"
            )
        return b"".join(chunks)
    except OSError as exc:
        raise PilotExactTaskPrBranchBoundReviewThreadCredentialCapabilityProductionBoundaryError(
            "branch-bound review-thread broker executable could not be read safely"
        ) from exc
    finally:
        os.close(descriptor)
...
def _load_broker_descriptor_at(
    ...
    *,
    ...
    try:
        ...
        raise PilotExactTaskPrBranchBoundReviewThreadCredentialCapabilityProductionBoundaryError(
            "branch-bound review-thread broker policy could not be read safely"
        ) from exc
    ...
    if broker_sha256 != policy["broker_executable_sha256"]:
        raise PilotExactTaskPrBranchBoundReviewThreadCredentialCapabilityProductionBoundaryError(
            "branch-bound review-thread broker executable hash does not match host policy"
        )
    return {
...
def _canonical_broker_descriptor(implementation: Any) -> Mapping[str, str]:
    try:
        ...
        raise PilotExactTaskPrBranchBoundReviewThreadCredentialCapabilityProductionBoundaryError(
            "branch-bound review-thread capability requires an elevated host operator"
        ) from exc
    return _load_broker_descriptor_at(
        _canonical_policy_path(), _canonical_broker_path(), require_host_control=True, implementation=implementation
    )


def install_pilot_exact_task_pr_branch_bound_review_thread_credential_capability_production_boundary(
    implementation: Any,
) -> None:
    if implementation is None:
        raise PilotExactTaskPrBranchBoundReviewThreadCredentialCapabilityProductionBoundaryError(
            "branch-bound review-thread capability implementation is unavailable"
        )
    marker = "_production_pilot_exact_task_pr_branch_bound_review_thread_credential_capability_boundary_installed"
    if getattr(implementation, marker, False):
        return
    private_materialize = implementation._materialize_verified_pilot_exact_task_pr_branch_bound_review_thread_credential_capability

    def materialize_pilot_exact_task_pr_branch_bound_review_thread_credential_capability(ruleset_observation: Any) -> Any:
        try:
            # Reject replay/stale/incomplete policy-source evidence before touching host broker state.
            implementation._require_live_ruleset_observation(ruleset_observation)
            descriptor = _canonical_broker_descriptor(implementation)
            return private_materialize(
                ruleset_observation=ruleset_observation,
                broker_descriptor=descriptor,
                now_provider=implementation._now_utc_seconds,
            )
        except implementation.PilotExactTaskPrBranchBoundReviewThreadCredentialCapabilityError:
            raise
        except (
            PilotExactTaskPrBranchBoundReviewThreadCredentialCapabilityProductionBoundaryError,
            ValueError, TypeError, AttributeError, OSError,
        ) as exc:
            raise implementation.PilotExactTaskPrBranchBoundReviewThreadCredentialCapabilityError(
                "host-controlled branch-bound review-thread capability failed closed"
            ) from exc

    implementation.materialize_pilot_exact_task_pr_branch_bound_review_thread_credential_capability = (
