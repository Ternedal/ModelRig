"""Host-pinned production boundary for ADR-DC-074 reviewer-write capability."""
...
PILOT_EXACT_TASK_PR_REVIEWER_WRITE_CREDENTIAL_BROKER_POLICY_SCHEMA = (
    "kaliv-rsi-pilot-exact-task-pr-reviewer-write-credential-broker-policy/v1"
)
...
class PilotExactTaskPrReviewerWriteCredentialCapabilityProductionBoundaryError(
    ValueError
):
    """Production reviewer-write broker is unsafe or unavailable."""
...
def _canonical_policy_path() -> Path:
    if os.name == "posix":
        return Path(
            ...
            "rsi-pilot-exact-task-pr-reviewer-write-credential-broker-policy-v1.json"
        )
    if os.name == "nt":
        return Path(
            r"C:\Program Files\ModelRig\DevControl\review"
            + r"\rsi-pilot-exact-task-pr-reviewer-write-credential-broker-policy-v1.json"
        )
    raise PilotExactTaskPrReviewerWriteCredentialCapabilityProductionBoundaryError(
        "reviewer-write broker policy platform is unsupported"
    )
...
def _canonical_broker_path() -> Path:
    if os.name == "posix":
        return Path(
            "/usr/local/libexec/modelrig/rsi-github-reviewer-write-broker-v1"
        )
    if os.name == "nt":
        return Path(
            r"C:\Program Files\ModelRig\DevControl\bin"
            + r"\rsi-github-reviewer-write-broker-v1.exe"
        )
    raise PilotExactTaskPrReviewerWriteCredentialCapabilityProductionBoundaryError(
        "reviewer-write broker executable platform is unsupported"
    )
...
def _canonical_bytes(value: Mapping[str, Any]) -> bytes:
    try:
        ...
        raise PilotExactTaskPrReviewerWriteCredentialCapabilityProductionBoundaryError(
            "reviewer-write broker policy is not canonical JSON"
        ) from exc
...
def _parse_policy(payload: bytes, *, broker_path: Path) -> Mapping[str, Any]:
    if not isinstance(payload, bytes) or not payload:
        raise PilotExactTaskPrReviewerWriteCredentialCapabilityProductionBoundaryError(
            "reviewer-write broker policy payload is missing"
        )
    try:
        value = json.loads(payload.decode("utf-8", errors="strict"))
    except (UnicodeError, json.JSONDecodeError) as exc:
        raise PilotExactTaskPrReviewerWriteCredentialCapabilityProductionBoundaryError(
            "reviewer-write broker policy is invalid JSON"
        ) from exc
    if not isinstance(value, Mapping) or set(value) != _POLICY_FIELDS:
        raise PilotExactTaskPrReviewerWriteCredentialCapabilityProductionBoundaryError(
            "reviewer-write broker policy fields mismatch"
        )

    expected = {
        "schema": PILOT_EXACT_TASK_PR_REVIEWER_WRITE_CREDENTIAL_BROKER_POLICY_SCHEMA,
        "provider": "github",
    ...
    if mismatch is not None:
        raise PilotExactTaskPrReviewerWriteCredentialCapabilityProductionBoundaryError(
            f"reviewer-write broker policy mismatch: {mismatch}"
        )
    ...
    if (
        ...
        raise PilotExactTaskPrReviewerWriteCredentialCapabilityProductionBoundaryError(
            "reviewer-write broker policy version/hash is invalid"
        )
    if payload != _canonical_bytes(dict(value)):
        raise PilotExactTaskPrReviewerWriteCredentialCapabilityProductionBoundaryError(
            "reviewer-write broker policy is not byte-for-byte canonical"
        )
    return dict(value)
...
def _read_broker_bytes(path: Path, *, require_host_control: bool) -> bytes:
    ...
    if not candidate.is_absolute() or _has_linkish_component(candidate):
        raise PilotExactTaskPrReviewerWriteCredentialCapabilityProductionBoundaryError(
            "reviewer-write broker executable path is unsafe"
        )
    flags = (
        os.O_RDONLY
    ...
    try:
        ...
        raise PilotExactTaskPrReviewerWriteCredentialCapabilityProductionBoundaryError(
            "reviewer-write broker executable is missing or unreadable"
        ) from exc
    try:
        ...
        if (
            ...
            raise PilotExactTaskPrReviewerWriteCredentialCapabilityProductionBoundaryError(
                "reviewer-write broker executable file is unsafe"
            )
        if require_host_control:
            try:
                ...
                raise PilotExactTaskPrReviewerWriteCredentialCapabilityProductionBoundaryError(
                    "reviewer-write broker executable is not host-admin controlled"
                ) from exc
        ...
        while remaining:
            ...
            if not chunk:
                raise PilotExactTaskPrReviewerWriteCredentialCapabilityProductionBoundaryError(
                    "reviewer-write broker executable read was incomplete"
                )
            chunks.append(chunk)
            remaining -= len(chunk)
        if os.read(descriptor, 1):
            raise PilotExactTaskPrReviewerWriteCredentialCapabilityProductionBoundaryError(
                "reviewer-write broker executable changed while reading"
            )
        ...
        if (
            ...
            raise PilotExactTaskPrReviewerWriteCredentialCapabilityProductionBoundaryError(
                "reviewer-write broker executable identity changed during read"
            )
        return b"".join(chunks)
    except OSError as exc:
        raise PilotExactTaskPrReviewerWriteCredentialCapabilityProductionBoundaryError(
            "reviewer-write broker executable could not be read safely"
        ) from exc
    finally:
        os.close(descriptor)
...
def _load_broker_descriptor_at(
    ...
    *,
    ...
    try:
        ...
        raise PilotExactTaskPrReviewerWriteCredentialCapabilityProductionBoundaryError(
            "reviewer-write broker policy could not be read safely"
        ) from exc
    ...
    if broker_sha256 != policy["broker_executable_sha256"]:
        raise PilotExactTaskPrReviewerWriteCredentialCapabilityProductionBoundaryError(
            "reviewer-write broker executable hash does not match host policy"
        )
    return {
        ...
        "request_path_template": str(policy["request_path_template"]),
    }
...
def _canonical_broker_descriptor() -> Mapping[str, str]:
    try:
        ...
        raise PilotExactTaskPrReviewerWriteCredentialCapabilityProductionBoundaryError(
            "reviewer-write capability requires an elevated host operator"
        ) from exc
...
def install_pilot_exact_task_pr_reviewer_write_credential_capability_production_boundary(
    implementation: Any,
) -> None:
    if implementation is None:
        raise PilotExactTaskPrReviewerWriteCredentialCapabilityProductionBoundaryError(
            "reviewer-write capability implementation is unavailable"
        )
    marker = (
        "_production_pilot_exact_task_pr_reviewer_write_credential_capability_boundary_installed"
    )
    if getattr(implementation, marker, False):
        return

    private_materialize = (
        implementation._materialize_verified_pilot_exact_task_pr_reviewer_write_credential_capability
    )

    def materialize_pilot_exact_task_pr_reviewer_write_credential_capability(
        reviewer_request_preflight: Any,
    ) -> Any:
        try:
            descriptor = _canonical_broker_descriptor()
            return private_materialize(
                reviewer_request_preflight=reviewer_request_preflight,
                broker_descriptor=descriptor,
                now_provider=implementation._now_utc_seconds,
            )
        except implementation.PilotExactTaskPrReviewerWriteCredentialCapabilityError:
            raise
        except (
            PilotExactTaskPrReviewerWriteCredentialCapabilityProductionBoundaryError,
            ValueError,
            TypeError,
            AttributeError,
            OSError,
        ) as exc:
            raise implementation.PilotExactTaskPrReviewerWriteCredentialCapabilityError(
                "host-controlled reviewer-write capability failed closed"
            ) from exc

    implementation.materialize_pilot_exact_task_pr_reviewer_write_credential_capability = (
