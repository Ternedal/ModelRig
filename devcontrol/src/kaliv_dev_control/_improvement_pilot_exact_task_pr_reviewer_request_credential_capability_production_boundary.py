"""Host-pinned production boundary for ADR-DC-071 reviewer-request credentials."""
...
_POLICY_FIELDS = {
    ...
    "secret_source",
    "secret_transport",
    "reviewer_requestability_read_required",
    "reviewer_request_write_required",
    "team_reviewer_write_forbidden",
    "self_review_forbidden",
    "pull_request_create_write_forbidden",
...
class PilotExactTaskPrReviewerRequestCredentialCapabilityProductionBoundaryError(
    ValueError
):
...
def _canonical_policy_path() -> Path:
    if os.name == "nt":
        return Path(
            "C:\\Program Files\\ModelRig\\DevControl\\publication\\"
    ...
    raise PilotExactTaskPrReviewerRequestCredentialCapabilityProductionBoundaryError(
        "reviewer-request credential-broker policy platform is unsupported"
    )
...
def _canonical_broker_path() -> Path:
    if os.name == "nt":
        return Path(
            "C:\\Program Files\\ModelRig\\DevControl\\bin\\"
            "rsi-github-pr-reviewer-request-broker-v1.exe"
        )
    if os.name == "posix":
        return Path(
            "/usr/local/libexec/modelrig/rsi-github-pr-reviewer-request-broker-v1"
        )
    raise PilotExactTaskPrReviewerRequestCredentialCapabilityProductionBoundaryError(
        "reviewer-request credential-broker executable platform is unsupported"
    )
...
def _canonical_bytes(value: Mapping[str, Any]) -> bytes:
    try:
        ...
        raise PilotExactTaskPrReviewerRequestCredentialCapabilityProductionBoundaryError(
            "reviewer-request credential-broker policy is not canonical JSON"
        ) from exc
...
def _parse_policy(payload: bytes, *, broker_path: Path) -> Mapping[str, Any]:
    if not isinstance(payload, bytes) or not payload:
        raise PilotExactTaskPrReviewerRequestCredentialCapabilityProductionBoundaryError(
            "reviewer-request credential-broker policy payload is missing"
        )
    try:
        value = json.loads(payload.decode("utf-8", errors="strict"))
    except (UnicodeError, json.JSONDecodeError) as exc:
        raise PilotExactTaskPrReviewerRequestCredentialCapabilityProductionBoundaryError(
            "reviewer-request credential-broker policy is invalid JSON"
        ) from exc
    if not isinstance(value, Mapping) or set(value) != _POLICY_FIELDS:
        raise PilotExactTaskPrReviewerRequestCredentialCapabilityProductionBoundaryError(
            "reviewer-request credential-broker policy fields mismatch"
        )
    expected = {
    ...
    if mismatch is not None:
        raise PilotExactTaskPrReviewerRequestCredentialCapabilityProductionBoundaryError(
            f"reviewer-request credential-broker policy mismatch: {mismatch}"
        )
    ...
    if (
        ...
        raise PilotExactTaskPrReviewerRequestCredentialCapabilityProductionBoundaryError(
            "reviewer-request credential-broker policy version/hash is invalid"
        )
    if payload != _canonical_bytes(dict(value)):
        raise PilotExactTaskPrReviewerRequestCredentialCapabilityProductionBoundaryError(
            "reviewer-request credential-broker policy is not byte-for-byte canonical"
        )
    return dict(value)
...
def _read_broker_bytes(path: Path, *, require_host_control: bool) -> bytes:
    ...
    if not candidate.is_absolute() or _has_linkish_component(candidate):
        raise PilotExactTaskPrReviewerRequestCredentialCapabilityProductionBoundaryError(
            "reviewer-request credential-broker executable path is unsafe"
        )
    flags = (
        os.O_RDONLY
    ...
    try:
        ...
        raise PilotExactTaskPrReviewerRequestCredentialCapabilityProductionBoundaryError(
            "reviewer-request credential-broker executable is missing or unreadable"
        ) from exc
    try:
        ...
        if (
            ...
            raise PilotExactTaskPrReviewerRequestCredentialCapabilityProductionBoundaryError(
                "reviewer-request credential-broker executable file is unsafe"
            )
        if os.name == "posix" and (observed.st_mode & 0o111) == 0:
            raise PilotExactTaskPrReviewerRequestCredentialCapabilityProductionBoundaryError(
                "reviewer-request credential-broker file is not executable"
            )
        if require_host_control:
            try:
                ...
                raise PilotExactTaskPrReviewerRequestCredentialCapabilityProductionBoundaryError(
                    "reviewer-request credential-broker executable is not host-admin controlled"
                ) from exc
        ...
        while remaining:
            ...
            if not chunk:
                raise PilotExactTaskPrReviewerRequestCredentialCapabilityProductionBoundaryError(
                    "reviewer-request credential-broker executable read was incomplete"
                )
            chunks.append(chunk)
            remaining -= len(chunk)
        if os.read(descriptor, 1):
            raise PilotExactTaskPrReviewerRequestCredentialCapabilityProductionBoundaryError(
                "reviewer-request credential-broker executable changed while reading"
            )
        ...
        if (
            ...
            raise PilotExactTaskPrReviewerRequestCredentialCapabilityProductionBoundaryError(
                "reviewer-request credential-broker executable identity changed during read"
            )
        return b"".join(chunks)
    except OSError as exc:
        raise PilotExactTaskPrReviewerRequestCredentialCapabilityProductionBoundaryError(
            "reviewer-request credential-broker executable could not be read safely"
        ) from exc
    finally:
        os.close(descriptor)
...
def _load_broker_descriptor_at(
    ...
    *,
    ...
    try:
        ...
        raise PilotExactTaskPrReviewerRequestCredentialCapabilityProductionBoundaryError(
            "reviewer-request credential-broker policy could not be read safely"
        ) from exc
    ...
    if broker_sha256 != policy["broker_executable_sha256"]:
        raise PilotExactTaskPrReviewerRequestCredentialCapabilityProductionBoundaryError(
            "reviewer-request credential-broker executable hash does not match host policy"
        )
    return {
        ...
        "operation_set": str(policy["operation_set"]),
...
def _canonical_broker_descriptor() -> Mapping[str, str]:
    try:
        ...
        raise PilotExactTaskPrReviewerRequestCredentialCapabilityProductionBoundaryError(
            "reviewer-request credential capability requires an elevated host operator"
        ) from exc
...
def install_pilot_exact_task_pr_reviewer_request_credential_capability_production_boundary(
    ...
    if implementation is None:
        raise PilotExactTaskPrReviewerRequestCredentialCapabilityProductionBoundaryError(
            "reviewer-request credential capability implementation is unavailable"
        )
    marker = (
    ...
    def materialize_pilot_exact_task_pr_reviewer_request_credential_capability(
        reviewer_identity_state_observation: Any,
    ) -> Any:
        try:
            ...
            raise implementation.PilotExactTaskPrReviewerRequestCredentialCapabilityError(
                "host-controlled reviewer-request credential capability failed closed"
            ) from exc
