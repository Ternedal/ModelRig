"""Host-pinned production boundary for ADR-DC-079 GraphQL review-thread capability."""
...
PILOT_EXACT_TASK_PR_REVIEW_THREAD_CREDENTIAL_BROKER_POLICY_SCHEMA = (
    "kaliv-rsi-pilot-exact-task-pr-review-thread-credential-broker-policy/v1"
)
...
class PilotExactTaskPrReviewThreadCredentialCapabilityProductionBoundaryError(ValueError):
    """Production review-thread broker is unsafe or unavailable."""
...
def _canonical_policy_path() -> Path:
    if os.name == "posix":
        return Path(
            ...
            "rsi-pilot-exact-task-pr-review-thread-credential-broker-policy-v1.json"
        )
    if os.name == "nt":
        return Path(
            r"C:\Program Files\ModelRig\DevControl\review"
            + r"\rsi-pilot-exact-task-pr-review-thread-credential-broker-policy-v1.json"
        )
    raise PilotExactTaskPrReviewThreadCredentialCapabilityProductionBoundaryError(
        "review-thread broker policy platform is unsupported"
    )
...
def _canonical_broker_path() -> Path:
    ...
    raise PilotExactTaskPrReviewThreadCredentialCapabilityProductionBoundaryError(
        "review-thread broker executable platform is unsupported"
    )
...
def _canonical_bytes(value: Mapping[str, Any]) -> bytes:
    try:
        ...
        raise PilotExactTaskPrReviewThreadCredentialCapabilityProductionBoundaryError(
            "review-thread broker policy is not canonical JSON"
        ) from exc
...
def _parse_policy(
    ...
    *,
    ...
    if not isinstance(payload, bytes) or not payload:
        raise PilotExactTaskPrReviewThreadCredentialCapabilityProductionBoundaryError(
            "review-thread broker policy payload is missing"
        )
    try:
        value = json.loads(payload.decode("utf-8", errors="strict"))
    except (UnicodeError, json.JSONDecodeError) as exc:
        raise PilotExactTaskPrReviewThreadCredentialCapabilityProductionBoundaryError(
            "review-thread broker policy is invalid JSON"
        ) from exc
    if not isinstance(value, Mapping) or set(value) != _POLICY_FIELDS:
        raise PilotExactTaskPrReviewThreadCredentialCapabilityProductionBoundaryError(
            "review-thread broker policy fields mismatch"
        )
    expected = {
        "schema": PILOT_EXACT_TASK_PR_REVIEW_THREAD_CREDENTIAL_BROKER_POLICY_SCHEMA,
        "provider": "github",
        "graphql_endpoint": implementation.PILOT_EXACT_TASK_PR_REVIEW_THREAD_GRAPHQL_ENDPOINT,
        "repository": "Ternedal/ModelRig",
        "credential_account": implementation.PILOT_EXACT_TASK_PR_REVIEW_THREAD_CREDENTIAL_ACCOUNT,
        "credential_protocol": implementation.PILOT_EXACT_TASK_PR_REVIEW_THREAD_CREDENTIAL_PROTOCOL,
        "operation": implementation.PILOT_EXACT_TASK_PR_REVIEW_THREAD_GRAPHQL_OPERATION,
        "broker_executable_path": os.fspath(broker_path),
        "secret_source": implementation.PILOT_EXACT_TASK_PR_REVIEW_THREAD_SECRET_SOURCE,
        "secret_transport": implementation.PILOT_EXACT_TASK_PR_REVIEW_THREAD_SECRET_TRANSPORT,
        "graphql_query_sha256": hashlib.sha256(
            implementation.PILOT_EXACT_TASK_PR_REVIEW_THREAD_GRAPHQL_QUERY.encode("utf-8")
        ).hexdigest(),
    ...
    if mismatch is not None:
        raise PilotExactTaskPrReviewThreadCredentialCapabilityProductionBoundaryError(
            f"review-thread broker policy mismatch: {mismatch}"
        )
    ...
    if (
        ...
        raise PilotExactTaskPrReviewThreadCredentialCapabilityProductionBoundaryError(
            "review-thread broker policy version/hash is invalid"
        )
    if payload != _canonical_bytes(dict(value)):
        raise PilotExactTaskPrReviewThreadCredentialCapabilityProductionBoundaryError(
            "review-thread broker policy is not byte-for-byte canonical"
        )
    return dict(value)
...
def _read_broker_bytes(path: Path, *, require_host_control: bool) -> bytes:
    ...
    if not candidate.is_absolute() or _has_linkish_component(candidate):
        raise PilotExactTaskPrReviewThreadCredentialCapabilityProductionBoundaryError(
            "review-thread broker executable path is unsafe"
        )
    ...
    try:
        ...
        raise PilotExactTaskPrReviewThreadCredentialCapabilityProductionBoundaryError(
            "review-thread broker executable is missing or unreadable"
        ) from exc
    try:
        ...
        if (
            ...
            raise PilotExactTaskPrReviewThreadCredentialCapabilityProductionBoundaryError(
                "review-thread broker executable file is unsafe"
            )
        if require_host_control:
            try:
                ...
                raise PilotExactTaskPrReviewThreadCredentialCapabilityProductionBoundaryError(
                    "review-thread broker executable is not host-admin controlled"
                ) from exc
        ...
        while remaining:
            ...
            if not chunk:
                raise PilotExactTaskPrReviewThreadCredentialCapabilityProductionBoundaryError(
                    "review-thread broker executable read was incomplete"
                )
            chunks.append(chunk)
            remaining -= len(chunk)
        if os.read(descriptor, 1):
            raise PilotExactTaskPrReviewThreadCredentialCapabilityProductionBoundaryError(
                "review-thread broker executable changed while reading"
            )
        ...
        if (observed.st_dev, observed.st_ino, observed.st_size) != (
            ...
            raise PilotExactTaskPrReviewThreadCredentialCapabilityProductionBoundaryError(
                "review-thread broker executable identity changed during read"
            )
        return b"".join(chunks)
    except OSError as exc:
        raise PilotExactTaskPrReviewThreadCredentialCapabilityProductionBoundaryError(
            "review-thread broker executable could not be read safely"
        ) from exc
    finally:
        os.close(descriptor)
...
def _load_broker_descriptor_at(
    ...
    *,
    ...
    try:
        ...
        raise PilotExactTaskPrReviewThreadCredentialCapabilityProductionBoundaryError(
            "review-thread broker policy could not be read safely"
        ) from exc
    ...
    if broker_sha256 != policy["broker_executable_sha256"]:
        raise PilotExactTaskPrReviewThreadCredentialCapabilityProductionBoundaryError(
            "review-thread broker executable hash does not match host policy"
        )
    return {
...
def _canonical_broker_descriptor(implementation: Any) -> Mapping[str, str]:
    try:
        ...
        raise PilotExactTaskPrReviewThreadCredentialCapabilityProductionBoundaryError(
            "review-thread capability requires an elevated host operator"
        ) from exc
...
def install_pilot_exact_task_pr_review_thread_credential_capability_production_boundary(
    implementation: Any,
) -> None:
    if implementation is None:
        raise PilotExactTaskPrReviewThreadCredentialCapabilityProductionBoundaryError(
            "review-thread capability implementation is unavailable"
        )
    marker = "_production_pilot_exact_task_pr_review_thread_credential_capability_boundary_installed"
    if getattr(implementation, marker, False):
        return
    private_materialize = (
        implementation._materialize_verified_pilot_exact_task_pr_review_thread_credential_capability
    )

    def materialize_pilot_exact_task_pr_review_thread_credential_capability(
        submitted_review_observation: Any,
    ) -> Any:
        try:
        ...
        except implementation.PilotExactTaskPrReviewThreadCredentialCapabilityError:
            raise
        except (
            PilotExactTaskPrReviewThreadCredentialCapabilityProductionBoundaryError,
            ValueError,
            TypeError,
            AttributeError,
            OSError,
        ) as exc:
            raise implementation.PilotExactTaskPrReviewThreadCredentialCapabilityError(
                "host-controlled review-thread capability failed closed"
            ) from exc

    implementation.materialize_pilot_exact_task_pr_review_thread_credential_capability = (
