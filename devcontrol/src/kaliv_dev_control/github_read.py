...
_MAX_RESPONSE_BYTES = 4000000

class GitHubReadError(RuntimeError):
    pass

def _valid_repository(value: Any) -> bool:
    if not isinstance(value, str) or not value or value.strip() != value or ('\x00' in value) or any((char.isspace() for char in value)) or (len(value.encode('utf-8')) > 200):
        return False
    parts = value.split('/')
...
class HttpResponse:
    ...
    def __post_init__(self) -> None:
        ...
        for key, value in self.headers.items():
            if not isinstance(key, str) or not key or (not isinstance(value, str)) or ('\r' in key + value) or ('\n' in key + value) or (len(key.encode('utf-8')) > 256) or (len(value.encode('utf-8')) > 8192):
...
class ReadOnlyTransport(Protocol):
    ...
    def get(self, url: str, *, headers: Mapping[str, str], timeout_seconds: int, max_bytes: int) -> HttpResponse:
        ...
...
class UrllibReadOnlyTransport:
    ...
    def get(self, url: str, *, headers: Mapping[str, str], timeout_seconds: int, max_bytes: int) -> HttpResponse:
        if isinstance(timeout_seconds, bool) or not isinstance(timeout_seconds, int) or isinstance(max_bytes, bool) or (not isinstance(max_bytes, int)) or (not 1 <= timeout_seconds <= 120) or (not 1 <= max_bytes <= 16000000):
            raise GitHubReadError('HTTP bounds are invalid')
        parsed = urllib.parse.urlsplit(url)
        if parsed.scheme != 'https' or parsed.hostname != _API_HOST or parsed.port not in {None, 443} or (parsed.username is not None) or (parsed.password is not None) or parsed.fragment:
        ...
        try:
            ...
            if len(body) > max_bytes:
                raise GitHubReadError('GitHub response exceeded the read budget')
            return HttpResponse(status=int(response.status), headers={key: value for key, value in response.headers.items()}, body=body)
        finally:
            response.close()
...
class GitHubReadReceipt:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.task_id, str) or _TASK_ID.fullmatch(self.task_id) is None or (not _valid_repository(self.repository)):
            raise GitHubReadError('GitHub receipt identity is invalid')
        if not isinstance(self.task_sha256, str) or _HEX64.fullmatch(self.task_sha256) is None:
            raise GitHubReadError('GitHub receipt task hash is invalid')
        if not isinstance(self.base_sha, str) or not isinstance(self.subject_sha, str) or _HEX40.fullmatch(self.base_sha) is None or (_HEX40.fullmatch(self.subject_sha) is None):
    ...
    def from_mapping(cls, value: Any) -> 'GitHubReadReceipt':
        ...
        fields = {'schema', 'task_id', 'task_sha256', 'repository', 'base_sha', 'operation', 'path', 'subject_sha', 'status', 'response_sha256', 'response_bytes', 'etag_sha256'}
        if set(value) != fields:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {'schema': self.schema, 'task_id': self.task_id, 'task_sha256': self.task_sha256, 'repository': self.repository, 'base_sha': self.base_sha, 'operation': self.operation, 'path': self.path, 'subject_sha': self.subject_sha, 'status': self.status, 'response_sha256': self.response_sha256, 'response_bytes': self.response_bytes, 'etag_sha256': self.etag_sha256}
    ...
    def verify_task(self, task: DevelopmentTask) -> None:
        ...
        expected = {'task_id': task.task_id, 'task_sha256': hashlib.sha256(task.canonical_json().encode('utf-8')).hexdigest(), 'repository': task.repository, 'base_sha': task.base_sha}
        actual = {'task_id': self.task_id, 'task_sha256': self.task_sha256, 'repository': self.repository, 'base_sha': self.base_sha}
        if actual != expected:
...
class GitHubReadAdapter:
    ...
    def _get(self, *, operation: str, suffix: str, path: str, query: Mapping[str, str] | None=None) -> tuple[Mapping[str, Any], HttpResponse]:
        if operation not in {'verify_base_commit', 'read_file'} or not suffix.startswith('/') or '?' in suffix or ('#' in suffix) or ('\\' in suffix):
            raise GitHubReadError('internal GitHub endpoint is invalid')
        url = 'https://' + _API_HOST + self._repository_path + suffix
        if query:
            if any((not isinstance(key, str) or not key or (not isinstance(value, str)) or (not value) for key, value in query.items())):
    ...
    def read_bytes(self, path: str, *, max_bytes: int=262144) -> tuple[bytes, GitHubReadReceipt]:
        ...
        if isinstance(declared_size, bool) or not isinstance(declared_size, int) or declared_size < 0 or (declared_size > max_bytes):
        ...
        receipt = self._receipt(operation='read_file', path=normalized, subject_sha=blob_sha, response=response)
        return (data, receipt)

    def read_text(self, path: str, *, max_bytes: int=262144) -> tuple[str, GitHubReadReceipt]:
