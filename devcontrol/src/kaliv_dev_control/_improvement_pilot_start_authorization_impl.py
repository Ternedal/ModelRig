...
ADR-DC-023 verifies only a short-lived human authorization over one fully
satisfied ADR-DC-022 runtime-preflight proof. It does not consume that
authorization, start product code, register commands, execute the pilot, mutate
Git/GitHub, or authorize remote/production activity.
"""
...
class PilotStartAuthorizationError(ValueError):
    """Pilot-start authorization is malformed, stale, untrusted or over-authorizing."""
...
def _require_satisfied_preflight(value: Any) -> PilotRuntimePreflightProof:
    if type(value) is not PilotRuntimePreflightProof:
        raise PilotStartAuthorizationError("exact PilotRuntimePreflightProof is required")
    if (
        value.authority != PILOT_RUNTIME_PREFLIGHT_PROOF_AUTHORITY
        ...
        raise PilotStartAuthorizationError(
            "pilot-start authorization requires a fully satisfied inert ADR-DC-022 proof"
        )
    try:
        replayed = PilotRuntimePreflightProof.from_mapping(value.to_dict())
    except Exception as exc:
        raise PilotStartAuthorizationError(
            "runtime preflight proof replay validation failed"
        ) from exc
    if replayed != value or replayed.sha256 != value.sha256:
        raise PilotStartAuthorizationError(
            "runtime preflight proof replay identity mismatch"
        )
    return value


def _expected_binding(preflight: PilotRuntimePreflightProof) -> dict[str, Any]:
    selection = preflight.selection_proof
    candidate = selection.selection.candidate_proof
    requirements = preflight.requirements
    return {
        "preflight_proof_sha256": preflight.sha256,
        "observation_sha256": preflight.observation_sha256,
        "selection_proof_sha256": preflight.selection_proof_sha256,
        "candidate_proof_sha256": preflight.candidate_proof_sha256,
        "requirements_sha256": preflight.requirements_sha256,
        "trial_scope_sha256": requirements.trial_scope_sha256,
        "runtime_receipt_sha256": preflight.observation.runtime_receipt_sha256,
...
class PilotStartAuthorization:
    ...
    preflight_proof: PilotRuntimePreflightProof
    ...
    def __post_init__(self) -> None:
        if self.schema != PILOT_START_AUTHORIZATION_SCHEMA:
            raise PilotStartAuthorizationError("pilot-start authorization schema unsupported")
        ...
        for name, value, pattern in (
            ...
            ("observation_sha256", self.observation_sha256, _HEX64),
        ...
        if type(self.local_commits_allowed) is not bool:
            raise PilotStartAuthorizationError("local_commits_allowed must be boolean")
        _notes(self.notes)
        expected = _expected_binding(preflight)
        if any(getattr(self, name) != item for name, item in expected.items()):
            raise PilotStartAuthorizationError(
                "pilot-start authorization does not exactly bind the satisfied preflight"
            )
        selection_actor = (
            preflight.selection_proof.selection.selection_maker_actor_id
        )
        ...
        if self.start_authorizer_actor_id == preflight.issuer_actor_id:
            raise PilotStartAuthorizationError(
                "pilot-start human authority must be distinct from the host preflight observer"
            )
        ...
        if expires <= authorized:
            raise PilotStartAuthorizationError("pilot-start authorization expiry is invalid")
        ...
        if self.human_start_intent != PILOT_START_AUTHORIZATION_INTENT:
            raise PilotStartAuthorizationError("pilot-start authorization intent unsupported")
        if (
    ...
    def from_mapping(cls, value: Any) -> "PilotStartAuthorization":
        data = dict(
            _strict(value, fields=_AUTHORIZATION_FIELDS, name="pilot-start authorization")
        )
        if not isinstance(data.get("preflight_proof"), Mapping):
            raise PilotStartAuthorizationError("preflight_proof must be an object")
        if not isinstance(data.get("notes"), list):
            raise PilotStartAuthorizationError("authorization notes must be an array")
        data["preflight_proof"] = PilotRuntimePreflightProof.from_mapping(
...
class PilotStartAuthorizationProof:
    ...
    def __post_init__(self) -> None:
        if self.schema != PILOT_START_AUTHORIZATION_PROOF_SCHEMA:
            raise PilotStartAuthorizationError("pilot-start authorization proof schema unsupported")
        for name, value in (
        ...
        if self.issuer_system_id != PILOT_START_AUTHORIZATION_ISSUER_SYSTEM_ID:
            raise PilotStartAuthorizationError("pilot-start proof issuer system is invalid")
        if type(self.authorization) is not PilotStartAuthorization:
            raise PilotStartAuthorizationError("exact PilotStartAuthorization is required")
        if self.authorization_sha256 != self.authorization.sha256:
            raise PilotStartAuthorizationError("pilot-start proof payload hash mismatch")
        for name in (
            "preflight_proof_sha256",
            "observation_sha256",
            "selection_proof_sha256",
        ...
        if self.issuer_actor_id != self.authorization.start_authorizer_actor_id:
            raise PilotStartAuthorizationError("pilot-start proof signer mismatch")
    ...
    def from_mapping(cls, value: Any) -> "PilotStartAuthorizationProof":
        data = dict(
            _strict(value, fields=_PROOF_FIELDS, name="pilot-start authorization proof")
        )
        if not isinstance(data.get("authorization"), Mapping):
            raise PilotStartAuthorizationError("authorization must be an object")
...
def _verify_pilot_start_authorization(
    *,
    ...
    if not isinstance(verifier, Ed25519AuthorityVerifier):
        raise PilotStartAuthorizationError("Ed25519 pilot-start verifier is required")
    ...
    return PilotStartAuthorizationProof(
        ...
        observation_sha256=authorization.observation_sha256,
        ...
        runtime_receipt_sha256=authorization.runtime_receipt_sha256,
