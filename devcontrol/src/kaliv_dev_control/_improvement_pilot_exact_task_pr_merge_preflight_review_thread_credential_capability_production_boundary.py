"""Host-pinned production boundary for ADR-DC-081 preflight review-thread capability."""
...
PILOT_EXACT_TASK_PR_MERGE_PREFLIGHT_REVIEW_THREAD_CREDENTIAL_BROKER_POLICY_SCHEMA = (
    "kaliv-rsi-pilot-exact-task-pr-merge-preflight-review-thread-credential-broker-policy/v1"
)
...
_POLICY_FIELDS = {
    ...
    "merge_preflight_required", "graphql_query_only", "graphql_mutation_forbidden",
    "graphql_introspection_forbidden", "rest_review_read_forbidden",
    ...
    "reviewer_request_write_forbidden", "pull_request_create_forbidden",
...
class PilotExactTaskPrMergePreflightReviewThreadCredentialCapabilityProductionBoundaryError(ValueError):
    """Production preflight review-thread broker is unsafe or unavailable."""
...
def _canonical_policy_path() -> Path:
    if os.name == "posix":
        return Path(
            ...
            "rsi-pilot-exact-task-pr-merge-preflight-review-thread-credential-broker-policy-v1.json"
        )
    if os.name == "nt":
        return Path(
            r"C:\Program Files\ModelRig\DevControl\review"
            + r"\rsi-pilot-exact-task-pr-merge-preflight-review-thread-credential-broker-policy-v1.json"
        )
    raise PilotExactTaskPrMergePreflightReviewThreadCredentialCapabilityProductionBoundaryError(
        "preflight review-thread broker policy platform is unsupported"
    )
...
def _canonical_broker_path() -> Path:
    ...
    raise PilotExactTaskPrMergePreflightReviewThreadCredentialCapabilityProductionBoundaryError(
        "preflight review-thread broker executable platform is unsupported"
    )
...
def _canonical_bytes(value: Mapping[str, Any]) -> bytes:
    try:
        ...
        raise PilotExactTaskPrMergePreflightReviewThreadCredentialCapabilityProductionBoundaryError(
            "preflight review-thread broker policy is not canonical JSON"
        ) from exc
...
def _parse_policy(
    ...
    *,
    ...
    if not isinstance(payload, bytes) or not payload:
        raise PilotExactTaskPrMergePreflightReviewThreadCredentialCapabilityProductionBoundaryError(
            "preflight review-thread broker policy payload is missing"
        )
    try:
        value = json.loads(payload.decode("utf-8", errors="strict"))
    except (UnicodeError, json.JSONDecodeError) as exc:
        raise PilotExactTaskPrMergePreflightReviewThreadCredentialCapabilityProductionBoundaryError(
            "preflight review-thread broker policy is invalid JSON"
        ) from exc
    if not isinstance(value, Mapping) or set(value) != _POLICY_FIELDS:
        raise PilotExactTaskPrMergePreflightReviewThreadCredentialCapabilityProductionBoundaryError(
            "preflight review-thread broker policy fields mismatch"
        )
    expected = {
        "schema": PILOT_EXACT_TASK_PR_MERGE_PREFLIGHT_REVIEW_THREAD_CREDENTIAL_BROKER_POLICY_SCHEMA,
        "provider": "github",
        "graphql_endpoint": implementation.PILOT_EXACT_TASK_PR_MERGE_PREFLIGHT_REVIEW_THREAD_GRAPHQL_ENDPOINT,
        "repository": "Ternedal/ModelRig",
        "credential_account": implementation.PILOT_EXACT_TASK_PR_MERGE_PREFLIGHT_REVIEW_THREAD_CREDENTIAL_ACCOUNT,
        "credential_protocol": implementation.PILOT_EXACT_TASK_PR_MERGE_PREFLIGHT_REVIEW_THREAD_CREDENTIAL_PROTOCOL,
        "operation": implementation.PILOT_EXACT_TASK_PR_MERGE_PREFLIGHT_REVIEW_THREAD_GRAPHQL_OPERATION,
        "broker_executable_path": os.fspath(broker_path),
        "secret_source": implementation.PILOT_EXACT_TASK_PR_MERGE_PREFLIGHT_REVIEW_THREAD_SECRET_SOURCE,
        "secret_transport": implementation.PILOT_EXACT_TASK_PR_MERGE_PREFLIGHT_REVIEW_THREAD_SECRET_TRANSPORT,
        "graphql_query_sha256": hashlib.sha256(
            implementation.PILOT_EXACT_TASK_PR_MERGE_PREFLIGHT_REVIEW_THREAD_GRAPHQL_QUERY.encode("utf-8")
        ).hexdigest(),
    ...
    if mismatch is not None:
        raise PilotExactTaskPrMergePreflightReviewThreadCredentialCapabilityProductionBoundaryError(
            f"preflight review-thread broker policy mismatch: {mismatch}"
        )
    ...
    if (
        ...
        raise PilotExactTaskPrMergePreflightReviewThreadCredentialCapabilityProductionBoundaryError(
            "preflight review-thread broker policy version/hash is invalid"
        )
    if payload != _canonical_bytes(dict(value)):
        raise PilotExactTaskPrMergePreflightReviewThreadCredentialCapabilityProductionBoundaryError(
            "preflight review-thread broker policy is not byte-for-byte canonical"
        )
    return dict(value)
...
def _read_broker_bytes(path: Path, *, require_host_control: bool) -> bytes:
    ...
    if not candidate.is_absolute() or _has_linkish_component(candidate):
        raise PilotExactTaskPrMergePreflightReviewThreadCredentialCapabilityProductionBoundaryError(
            "preflight review-thread broker executable path is unsafe"
        )
    ...
    try:
        ...
        raise PilotExactTaskPrMergePreflightReviewThreadCredentialCapabilityProductionBoundaryError(
            "preflight review-thread broker executable is missing or unreadable"
        ) from exc
    try:
        ...
        if (
            ...
            raise PilotExactTaskPrMergePreflightReviewThreadCredentialCapabilityProductionBoundaryError(
                "preflight review-thread broker executable file is unsafe"
            )
        if require_host_control:
            try:
                ...
                raise PilotExactTaskPrMergePreflightReviewThreadCredentialCapabilityProductionBoundaryError(
                    "preflight review-thread broker executable is not host-admin controlled"
                ) from exc
        ...
        while remaining:
            ...
            if not chunk:
                raise PilotExactTaskPrMergePreflightReviewThreadCredentialCapabilityProductionBoundaryError(
                    "preflight review-thread broker executable read was incomplete"
                )
            chunks.append(chunk)
            remaining -= len(chunk)
        if os.read(descriptor, 1):
            raise PilotExactTaskPrMergePreflightReviewThreadCredentialCapabilityProductionBoundaryError(
                "preflight review-thread broker executable changed while reading"
            )
        ...
        if (observed.st_dev, observed.st_ino, observed.st_size) != (
            ...
            raise PilotExactTaskPrMergePreflightReviewThreadCredentialCapabilityProductionBoundaryError(
                "preflight review-thread broker executable identity changed during read"
            )
        return b"".join(chunks)
    except OSError as exc:
        raise PilotExactTaskPrMergePreflightReviewThreadCredentialCapabilityProductionBoundaryError(
            "preflight review-thread broker executable could not be read safely"
        ) from exc
    finally:
        os.close(descriptor)
...
def _load_broker_descriptor_at(
    ...
    *,
    ...
    try:
        ...
        raise PilotExactTaskPrMergePreflightReviewThreadCredentialCapabilityProductionBoundaryError(
            "preflight review-thread broker policy could not be read safely"
        ) from exc
    ...
    if broker_sha256 != policy["broker_executable_sha256"]:
        raise PilotExactTaskPrMergePreflightReviewThreadCredentialCapabilityProductionBoundaryError(
            "preflight review-thread broker executable hash does not match host policy"
        )
    return {
...
def _canonical_broker_descriptor(implementation: Any) -> Mapping[str, str]:
    try:
        ...
        raise PilotExactTaskPrMergePreflightReviewThreadCredentialCapabilityProductionBoundaryError(
            "preflight review-thread capability requires an elevated host operator"
        ) from exc
...
def install_pilot_exact_task_pr_merge_preflight_review_thread_credential_capability_production_boundary(
    implementation: Any,
) -> None:
    if implementation is None:
        raise PilotExactTaskPrMergePreflightReviewThreadCredentialCapabilityProductionBoundaryError(
            "preflight review-thread capability implementation is unavailable"
        )
    marker = (
        "_production_pilot_exact_task_pr_merge_preflight_review_thread_credential_capability_boundary_installed"
    )
    if getattr(implementation, marker, False):
        return
    private_materialize = (
        implementation._materialize_verified_pilot_exact_task_pr_merge_preflight_review_thread_credential_capability
    )
    ...
    def materialize_pilot_exact_task_pr_merge_preflight_review_thread_credential_capability(
        ...
        try:
            # Reject stale/reloaded/incomplete ADR-080 evidence before touching
            # host broker policy or binary.
            implementation._require_live_merge_preflight(merge_preflight)
        ...
        except implementation.PilotExactTaskPrMergePreflightReviewThreadCredentialCapabilityError:
            raise
        except (
            PilotExactTaskPrMergePreflightReviewThreadCredentialCapabilityProductionBoundaryError,
            ValueError,
            TypeError,
            AttributeError,
            OSError,
        ) as exc:
            raise implementation.PilotExactTaskPrMergePreflightReviewThreadCredentialCapabilityError(
                "host-controlled preflight review-thread capability failed closed"
            ) from exc

    implementation.materialize_pilot_exact_task_pr_merge_preflight_review_thread_credential_capability = (
