...
Version **1.54.0** — "STREAMENDE VOICE (V6-kernen): kuren mod voice-langsomhed med store cloud-modeller. Pipelinen splittede ALLEREDE paa saetningsgraenser + TTS pr. saetning, men endpointet bufrede alt til ét svar. Nu: converse() faar on_transcript/on_chunk-callbacks; nyt /voice/converse/stream-endpoint (NDJSON: transcript → chunk pr. saetning m. base64-lyd → done); Go-rute (proxy flusher allerede); app konsumerer stream + afspiller hver saetning straks via en Channel-kø mens resten genereres, og fylder assistent-boblen sætning for sætning. Kaliv taler første saetning med det samme i stedet for at vente paa hele svaret. 8 nye tests, bagud-kompatibel. Verificeret: build+tests. ON-DEVICE afspilning = Anders". Follows 1.53.0. Autonomous sessions, **2026-07-02 → 07-12**.
