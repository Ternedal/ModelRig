...
Version **1.55.0** — "Selv-audit af den streamende voice (v1.54.0) fandt 3 samtidigheds-bugs unit-tests ikke fanger: (1) RMS-meter-polling gik TABT i omskrivningen → baren frøs paa 0 under tale (barge-in-detektion virkede stadig, men ikke visningen); (2) RACE i replyIdx — sat i en async launch, saa en hurtig foerste saetning kunne miste sin tekst; (3) skroebelig transcript-gendannelse via messages-indeks. Fix: meter-polling genindfoert, replyIdx sat synkront, transcript i plain var, direkte snapshot-mutation (messages er traad-sikker SnapshotStateList), tom-svar-boble fjernes. Verificeret: build+tests". Follows 1.54.0. Autonomous sessions, **2026-07-02 → 07-12**.
