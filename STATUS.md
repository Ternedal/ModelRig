...
Version **1.35.0** — "DESKTOP-KAERLIGHED (V6.6 a–d): desktop er nu KALIV, ikke ModelRig. (a) Kaliv-paletten porteret 1:1 fra Android som dark+light med toggle (samme LocalKalivColors-moenster, samme Markdown-linkColor-lektion ramte og blev fixet samme vej); (b) TOOLS paa desktop: tools-switch, non-streaming tools-chat, bekraeftelseskort (Godkend/Afvis — gate haandhaeves i workeren) og Handlingslog-dialog mod /tools/audit; (c) rigtig parring (pair/start+claim, dev-mode) i stedet for env-var/manuel token, ALLE indstillinger persisteres nu i SQLite (foer: glemte alt mellem starter); (d) friendlyError-mapping porteret. packageName+vinduestitel=Kaliv; kun backenden hedder ModelRig. Compile-verificeret". Follows 1.34.17. Autonomous sessions, **2026-07-02 → 07-10**.
