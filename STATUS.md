...
Version **1.34.16** — "Haerdning (A) + migration (C). A: jagtede flere af gaars moenstre. Fandt (1) alle Go string-env-reads UDEN trim — samme mellemrums-footgun som bind-buggen, nu trimmet; (2) backup laeste device-tokens fra FORKERT env-var (MODELRIG_DATA_PATH vs serverens MODELRIG_DATA) + relativ sti, saa en restore ikke ville gendanne parringen — rettet til at matche serveren. C: migrate_data.py samler gamle relative data-filer ind i data-roden, dry-run-default, klobrer aldrig en nyere fil, 7 tests. Go env-trim-test + migrate-test mutationstjekket, i CI". Follows 1.34.15 (data-rod). Autonomous sessions, **2026-07-02 → 07-10**.
