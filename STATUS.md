...
Version **1.34.17** — "Cloud-tools (A+B). A: verificeret at cloud-tools-stien er trådet korrekt ende-til-ende (appen sender cloudKey/base_url paa TOOLS-stien, ikke kun chat; gaten koerer lokalt uanset origin; writes kraever kort med origin=cloud). Ny T30-test beviser hele stien med stubbet upstream, mutationstjekket. B: designet auto-rute-til-cloud-naar-tools-paa (opt-in switch, manuel vinder, kun hvis cloud-noegle, tydelig chip, privacy-variant uden RAG) i CLOUD_TOOLS.md + on-device test-protokol. Ingen ny cloud-integration noedvendig for Ollama Cloud — flowet fandtes". Follows 1.34.16 (haerdning). Autonomous sessions, **2026-07-02 → 07-10**.
