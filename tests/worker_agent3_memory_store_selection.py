...
def configured(
    ...
    *,
    mode: str | None,
    signing_material: bytes | None = SIGNING_MATERIAL,
):
    names = {
        ...
        "KALIV_AGENT3_TASK_PLAN_DB": str(tmp / "task-plans.db"),
        "KALIV_AGENT3_APPROVAL_DB": str(tmp / "approvals.db"),
    }
    if mode is not None:
        names["KALIV_AGENT3_MEMORY_STORE"] = mode
    if signing_material is not None:
        names["KALIV_AGENT3_MEMORY_API_SECRET"] = signing_material.decode("ascii")
    touched = set(names) | {
...
with tempfile.TemporaryDirectory(prefix="kaliv-t033-selection-incomplete-") as raw:
    ...
    with configured(tmp, mode="protected"):
        app = FastAPI()
        before_routes = tuple(app.routes)
        before_middleware = tuple(app.user_middleware)
        try:
            mount_agent3(app, protected_provider_factory=TestAeadProvider)
        except Exception:
...
with tempfile.TemporaryDirectory(prefix="kaliv-t033-selection-signing-") as raw:
    tmp = Path(raw)
    provider_calls = 0
    ...
    with configured(tmp, mode="protected", signing_material=None):
        app = FastAPI()
        before_routes = tuple(app.routes)
        before_middleware = tuple(app.user_middleware)
        try:
            ...
            check(True, "missing gateway signing material aborts protected startup")
        else:
            check(False, "missing gateway signing material aborts protected startup")
        check(
            provider_calls == 0,
            "signing-material failure occurs before provider or database selection",
        )
        check(
...
with tempfile.TemporaryDirectory(prefix="kaliv-t033-selection-legacy-") as raw:
    tmp = Path(raw)
    with configured(tmp, mode=None, signing_material=None):
        app = FastAPI()
