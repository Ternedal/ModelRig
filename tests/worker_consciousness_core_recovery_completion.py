#!/usr/bin/env python3
"""C29-J process-local recovery completion tests."""
...
def sample(marker, wall, mono, seq, epoch):
    return ClockSample(
        ...
        source_ref="trusted:c29j:" + marker,
        confidence=1.0,
...
class CaptureEngine:
    ...
    async def think(self, request, cognitive_profile, *, context=None):
        ...
        payload["interpretation"] = "C29-J recovery completion."
...
class RecoveryCompletionTests(unittest.TestCase):
    def durable(self):
        authority = SelfBootstrapAuthority(
            ...
            authority_ref="operator:test:c29j",
            source_refs=["registry:test:c29j"],
            production_activation=False,
        )
        state = bootstrap_self_state(
            authority,
            personality_state_ref="personality-state:c29j",
            world_state_ref="world-state:c29j",
            workspace_ref="workspace:c29j",
            affect=SelfAffect(
                labels=["focused"],
                valence=0.0,
                arousal=0.2,
                confidence=1.0,
                source_refs=["affect:c29j"],
            ),
        )
        return state
    ...
    def person(self, state):
        return ActivePersonBindingSnapshot(
            ...
            body_source_ref="body:c29j",
            voice_source_ref="voice:c29j",
            registry_source_ref="registry:c29j",
            production_activation=False,
        )
    ...
    def context(self, state):
        return bootstrap_runtime_session(
            ...
            bootstrap_source_ref="runtime:c29j",
    ...
    def profile(self):
        return CognitiveProfile(
            ...
            profile_id="cog-" + "7" * 32,
            engine_instance_id="engine:test:c29j",
            provider="mock",
            model="c29j-model",
            reasoning_depth=0.8,
    ...
    def event(self, marker, sequence):
        return CognitionEvent(
            ...
            source_ref="operator:c29j:" + marker,
            summary="C29-J explicit cognition event.",
            salience=1.0,
    ...
    def test_primitive_requires_consumed_matching_window(self):
        state = self.durable()
        context = self.context(state)
        active = open_continuity_reorientation_window(
        ...
        receipt = build_continuity_recovery_completion(
            continuity_state=context.continuity_state,
        ...
        self.assertTrue(receipt.continuity_context_consumed)
        self.assertEqual(
            receipt.accepted_cycle_id,
            "cycle-" + "9" * 32,
        )
        self.assertEqual(receipt.transition_receipt_ref, "transition:test")
        ...
        self.assertTrue(
            continuity_recovery_completion_ref(receipt).startswith(
                "continuity-recovery-completion:"
            )
        )

    def test_idle_has_no_completion(self):
        state = self.durable()
    ...
    def test_first_run_publishes_exact_completion(self):
        state = self.durable()
        session, engine = self.session(state)
        ...
        receipt = session.recovery_completion
        ...
        self.assertEqual(
            receipt.transition_receipt_ref,
            session.live_state.last_transition_receipt_ref,
        )
        self.assertEqual(
            receipt.continuity_state_ref,
            session.continuity_reorientation_window.continuity_state_ref,
        )
        self.assertEqual(
            receipt.wake_receipt_ref,
            session.continuity_state.wake_receipt_ref,
        )
        self.assertEqual(
            session.continuity_reorientation_window.state,
            "CONSUMED",
    ...
    def test_later_runs_do_not_replace_completion(self):
        state = self.durable()
        session, engine = self.session(state)

        session.submit(self.event("7", 1))
        run(session.step(profile=self.profile()))
        first_receipt = session.recovery_completion
        first_ref = continuity_recovery_completion_ref(first_receipt)
    ...
    def test_close_clears_process_local_completion(self):
        state = self.durable()
