...
LEAK_MARKERS = (
    "T033-DPAPI-LEAK-LOCAL-8f3157b2",
    "T033-DPAPI-LEAK-LOCAL-SOURCE-904268c3",
    "T033-DPAPI-LEAK-MANAGEMENT-a15379d4",
    "T033-DPAPI-LEAK-MANAGEMENT-SOURCE-b2648ae5",
)
...
with tempfile.TemporaryDirectory(prefix="kaliv-t033-dpapi-leak-") as tmp:
    ...
    secret = writer.create(
        ...
        value=LEAK_MARKERS[2],
        sensitivity="secret",
        source_ref=LEAK_MARKERS[3],
    )
    writer.close()
    ...
    check(
        ...
        opened.value == LEAK_MARKERS[2] and opened.source_ref == LEAK_MARKERS[3],
    )
    check(
    ...
    check(
        ...
        all(value not in rendered for value in LEAK_MARKERS),
    )
