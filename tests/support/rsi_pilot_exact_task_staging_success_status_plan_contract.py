...
# ADR-084 live provenance keeps only weak references to its ADR-083 source.
# Downstream qualification helpers intentionally cross function boundaries, so
# keep the exact live pair strongly reachable for this isolated test process.
_LIVE_BUILD_IDENTITY_KEEPALIVE: list[tuple[object, object]] = []
...
def _live_build_identity(*, recovered=False):
    ...
    _LIVE_BUILD_IDENTITY_KEEPALIVE.append((runtime, receipt))
