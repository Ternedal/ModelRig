...
def run_contract() -> None:
    ...
    # ADR-DC-054 derives one deterministic draft-PR plan and verifies human intent.
    # It still performs no GitHub PR mutation and grants no reusable PR authority.
    ...
    # ADR-DC-058 performs one exact draft-only create through the pinned broker,
    # then verifies it read-only and retains no reusable PR mutation authority.
    ...
    # ADR-DC-062 freshly re-observes the exact signed draft after nonce burn and
    # fails closed on any updated_at/head/base/metadata drift. No mutation occurs.
