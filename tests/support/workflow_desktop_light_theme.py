#!/usr/bin/env python3
"""Focused desktop light-theme authority for #779 pkt. 5 / #1270.

This support gate measures the product boundary that the generic token contrast
gate cannot see: which token roles desktop Brand.kt actually binds to its light
palette and Material3 field/container colors. It is invoked by the existing
top-level workflow_design_token_contrast.py gate so adding this focused slice
does not silently change CURRENT_STATE's top-level test inventory authority.
"""
