...
Run on the dedicated Windows gate with:
``PYTHONPATH=worker python tests/support/windows_job_contract.py``.
The deterministic API-contract tests and the native kernel probes share one
entrypoint; this support suite is wired explicitly rather than pretending it is
part of the root tests/*.py glob documented by CURRENT_STATE.md.
"""
...
check(
    limits.ui_restrictions == DEFAULT_UI_RESTRICTIONS,
    "Tier A receives the complete UI-restriction mask by default",
)
rejects(
    lambda: JobLimits(process_memory_bytes=True),
    "memory",
    "bool is not a memory limit",
)
rejects(
    lambda: JobLimits(process_memory_bytes=32 * 1024 * 1024),
    "64 MiB",
    "too-small memory limit is rejected",
)
rejects(
    lambda: JobLimits(active_process_limit=0),
    "1..64",
    "zero-process job is rejected",
)
rejects(
    lambda: JobLimits(ui_restrictions=0x100),
    "mask",
    "unknown UI restriction bits are rejected",
)
...
check(
    native.BasicLimitInformation.ActiveProcessLimit == 4,
    "active-process count reaches the native structure",
)
check(
    native.ProcessMemoryLimit == 256 * 1024 * 1024,
...
check(
    [call[0] for call in api.calls] == ["create", "configure", "assign", "resume"],
    "process is configured and assigned before it is resumed",
)
job.terminate(9)
job.close()
check(
    [call[0] for call in api.calls][-2:] == ["terminate", "close"],
    "termination precedes handle close",
)
job.close()
check(
    [call[0] for call in api.calls].count("close") == 1,
...
check(
    created[0][1]["creationflags"]
    & getattr(subprocess, "CREATE_SUSPENDED", 0x4),
    "child is born suspended",
)
check(
    getattr(proc, "_kaliv_windows_job", None) is not None,
    "successful spawn retains the Job Object until output is consumed",
)
check(
    terminate_attached_job(proc, 7),
    "attached tree can be terminated as one unit",
)
check(
    getattr(proc, "_kaliv_windows_job", None) is None,
...
try:
    spawn_in_job(
        ...
        env={},
        limits=limits,
        popen_factory=lambda *a, **kw: bad,
        api=api,
    )
    check(False, "resume failure must abort spawn")
except WindowsIsolationError:
    check(
        bad.killed == 1 and bad.waited == 1,
        "resume failure kills and reaps the still-suspended process",
    )
    check(
        "terminate" in [c[0] for c in api.calls]
        and "close" in [c[0] for c in api.calls],
        "resume failure also destroys the Job Object",
    )

rejects(
    lambda: windows_creationflags(True),
    "flags",
    "boolean creation flags are rejected",
)
...
if os.name != "nt":
    ...
    check(
        p.returncode == 0 and out.decode().strip() == "native-ok",
    ...
    close_attached_job(p)
    check(
        "memory-denied" in out.decode("utf-8", "replace"),
    ...
    close_attached_job(p)
    check(
        "process-limit-denied" in out.decode("utf-8", "replace"),
