...
def run_contract():
    ...
    parent.run_contract()
    cap,strict,req,live_source,items,temp,broker,broker_bytes=parent._source()
    try:
        ...
        source=policy._get_live_pr_review_thread_policy_inputs(result)
        assert source is not None and source["review_thread_state_observation"] is obs and source["ruleset_applicability"] is app
