...
executor-capability, snapshot, plan, one-shot execution and verification contracts execute, then cleans up.
"""
...
import rsi_pilot_exact_task_product_pilot_execution_verification_contract as execution_verification_contract  # noqa: E402
...
def run_contract() -> None:
    ...
    try:
        ...
        execution_verification_contract.run_contract(shared_fixture=fixture)
        verification_done = time.monotonic()
        print(
            f"ADR-DC-109 execution-verification contract completed in {verification_done - execution_done:.1f}s",
            flush=True,
        )
        execution_session_contract.run_contract(shared_fixture=fixture)
        session_done = time.monotonic()
        print(
            f"product-pilot execution-session facade contract completed in {session_done - verification_done:.1f}s",
            flush=True,
        )
        execution_recovery_contract.run_contract(shared_fixture=fixture)
        recovery_done = time.monotonic()
        print(
            f"ADR-DC-110 execution-recovery contract completed in {recovery_done - session_done:.1f}s",
            flush=True,
        )
        execution_recovery_resolution_contract.run_contract(shared_fixture=fixture)
