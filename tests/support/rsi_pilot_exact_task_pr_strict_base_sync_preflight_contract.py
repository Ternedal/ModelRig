"""Adversarial contract for ADR-DC-088 strict base-sync preflight."""
...
from kaliv_dev_control import improvement_pilot_exact_task_pr_status_evaluated_review_thread_read_requirements as requirements  # noqa: E402
from kaliv_dev_control.github_read import HttpResponse  # noqa: E402
import rsi_pilot_exact_task_pr_status_evaluated_review_thread_read_requirements_contract as parent  # noqa: E402

SCHEMA = ROOT / "devcontrol" / "schemas" / "rsi-pilot-exact-task-pr-strict-base-sync-preflight-v1.schema.json"
...
def _reject(fn) -> None:
    try:
        ...
        requirements.PilotExactTaskPrStatusEvaluatedReviewThreadReadRequirementsError,
        ValueError,
        TypeError,
        OSError,
        AssertionError,
        ...
        return
    raise AssertionError("ADR-DC-088 unexpectedly accepted unsafe strict-sync evidence")
...
def _response(value, *, etag: str) -> HttpResponse:
    ...
    return HttpResponse(
        status=200,
        headers={"content-type": "application/vnd.github+json", "etag": etag},
        body=payload,
    )
...
def _compare(
    ...
    *,
    ...
    return {
        ...
        "merge_base_commit": {"sha": base_sha if merge_base_sha is None else merge_base_sha},
        "commits": [],
        "files": [],
    }
...
def _transport(source, *, first_tip="b" * 40, second_tip=None, compare_doc=None):
    second_tip = first_tip if second_tip is None else second_tip
...
def _live_requirements():
    items = parent._chain()
    evaluated, applicability, *_ = items
    result = requirements.define_pilot_exact_task_pr_status_evaluated_review_thread_read_requirements(
        evaluated,
        applicability,
    )
...
def _observe(
    ...
    transport,
    *,
    times=("2026-09-15T19:30:00Z", "2026-09-15T19:30:03Z"),
):
    clock = iter(times)
    return sync._observe_verified_pilot_exact_task_pr_strict_base_sync_preflight(
        review_thread_read_requirements=source,
        transport=transport,
...
def run_contract() -> None:
    ...
    parent.run_contract()
    source, items = _live_requirements()
    try:
        live_requirements = requirements._get_live_pr_status_evaluated_review_thread_read_requirements_inputs(source)
        assert live_requirements is not None
        evaluated = live_requirements["required_status_evaluation"]

        transport = _transport(source)
        result = _observe(source, transport)
        assert result.preflight_authenticated is True
        assert result.review_thread_read_requirements_sha256 == source.sha256
        ...
        assert result.ruleset_applicability_sha256 == source.ruleset_applicability_sha256
        assert result.ruleset_observation_sha256 == source.ruleset_observation_sha256
        assert result.status_check_observation_sha256 == source.status_check_observation_sha256
        assert result.review_thread_graphql_query_sha256 == source.graphql_query_sha256
        ...
        assert result.base_commit_sha == result.merge_base_sha == "b" * 40
        assert result.compare_status == "ahead"
        assert result.ahead_by == 3 and result.behind_by == 0 and result.total_commits == 3

        for field in (
            "source_review_thread_requirements_verified",
            "source_status_evaluation_verified",
            "source_required_status_pass_verified",
            "review_thread_query_requirements_preserved",
            ...
            "review_thread_read_capability_required",
        ...
        assert transport.calls[0][0].endswith("/branches/main")
        assert (
            f"/compare/{'b' * 40}...{source.predicted_commit_sha}?per_page=1&page=1"
            in transport.calls[1][0]
        )
        ...
        replay = sync.PilotExactTaskPrStrictBaseSyncPreflight.from_mapping(result.to_dict())
        assert replay == result and replay.sha256 == result.sha256
        assert replay.preflight_authenticated is False

        loose = requirements.PilotExactTaskPrStatusEvaluatedReviewThreadReadRequirements.from_mapping(source.to_dict())
        assert loose.requirements_authenticated is False
        untouched = _Transport([])
        _reject(lambda: _observe(loose, untouched))
        assert untouched.calls == []

        _reject(lambda: _observe(source, _transport(source, second_tip="c" * 40)))
        _reject(
            lambda: _observe(
                source,
                _transport(
        ...
        _reject(
            lambda: _observe(
                source,
                _transport(source, compare_doc=_compare("b" * 40, ahead_by=2, behind_by=1)),
            )
        )
        _reject(
            lambda: _observe(
                source,
                _transport(source, compare_doc=_compare("b" * 40, merge_base_sha="c" * 40)),
            )
        )
        _reject(
            lambda: _observe(
                source,
                _transport(source, compare_doc=_compare("b" * 40, base_commit_sha="c" * 40)),
            )
        )
        _reject(
            lambda: _observe(
                source,
                _transport(source, compare_doc=_compare("b" * 40, ahead_by=3, total_commits=4)),
            )
        )
        _reject(
            lambda: _observe(
                source,
                _transport(source),
                times=("2026-09-15T19:30:00Z", "2026-09-15T19:30:11Z"),
            )
        )
        _reject(
            lambda: _observe(
                source,
                _transport(source),
                times=("2026-09-15T19:30:03Z", "2026-09-15T19:30:02Z"),
            )
        )

        wrong_branch = _Transport(
            [_response({"name": "not-main", "commit": {"sha": "b" * 40}}, etag='W/"wrong"')]
        )
        ...
        tampered["review_thread_read_requirements_sha256"] = "1" * 64
        _reject(lambda: sync.PilotExactTaskPrStrictBaseSyncPreflight.from_mapping(tampered))
        tampered = result.to_dict()
        tampered["merge_authorized"] = True
        _reject(lambda: sync.PilotExactTaskPrStrictBaseSyncPreflight.from_mapping(tampered))
        tampered = result.to_dict()
        tampered["compare_request_url_sha256"] = "2" * 64
        _reject(lambda: sync.PilotExactTaskPrStrictBaseSyncPreflight.from_mapping(tampered))
        ...
        assert schema["properties"]["source_review_thread_requirements_verified"]["const"] is True
        ...
        assert tuple(
            inspect.signature(sync.observe_pilot_exact_task_pr_strict_base_sync_preflight).parameters
        ) == ("review_thread_read_requirements",)
        ...
        assert "status_evaluated_review_thread_read_requirements" in module_source
