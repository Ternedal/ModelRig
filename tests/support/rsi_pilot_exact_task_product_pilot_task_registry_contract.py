...
def _build(lineage_receipt, proof, when="2026-09-15T09:54:20Z"):
...
def run_contract() -> None:
    ...
    try:
        ...
        receipt = _build(lineage_receipt, proof)
        ...
        _reject(lambda: _build(lineage_receipt, proof, "2026-09-15T09:55:12Z"))

        wrong_workspace = replace(proof, workspace_root_path_sha256="f" * 64)
        _reject(lambda: _build(lineage_receipt, wrong_workspace))

        wrong_tasks = replace(proof, allowed_task_ids=("another-task",))
        _reject(lambda: _build(lineage_receipt, wrong_tasks))

        no_go = replace(proof, decision="no_go", pilot_go_authorized=False, notes=("stop",))
        _reject(lambda: _build(lineage_receipt, no_go))
