"""Adversarial contract for hardened ADR-DC-076 post-request attestation."""
...
def _live_transaction():
    capability, _path, _raw, write_temp, parent_temp, cleanup = (
        parent._live_capability()
    )
...
def _reader(value, *, drift_second: bool = False, teams: int = 0):
    calls = []
...
def run_contract() -> None:
    ...
    try:
        ...
        assert result.reviewer_write_credential_capability_sha256 == value.reviewer_write_credential_capability_sha256
        ...
        team_reader, team_calls = _reader(value, teams=1)
        _reject(lambda: _attest(value, team_reader))
        ...
        assert "requested_team_count" in source
    finally:
        ledger_temp.cleanup()
        write_temp.cleanup()
        parent_temp.cleanup()
