...
class EpisodeReviewLifecycleTests(unittest.TestCase):
    ...
    def test_disabled_runtime_does_not_delete_unowned_starlette_state(self):
        @asynccontextmanager
        async def inner(_app):
            yield

        app = SimpleNamespace(state=State({"sentinel": "keep"}))
        lifespan = compose_episode_review_lifespan(
            inner,
            runtime_factory=lambda _app: None,
        )

        async def exercise():
            async with lifespan(app):
                self.assertEqual(app.state.sentinel, "keep")
        ...
        for name in (
            ...
            self.assertFalse(hasattr(app.state, name), name)
