#!/usr/bin/env python3
"""A4-18 physical operator and receipt safety contracts."""
...
class Agent4PhysicalReadOperatorTests(unittest.TestCase):
    def sources(self) -> dict[str, str]:
        return {
            ...
            for path in (COMMON, PROCESS, RECORD, FINALIZE, WRAPPER, CORE_COMPAT)
        }
    ...
    def test_wrapper_routes_responsibilities_to_single_entrypoints(self) -> None:
        ...
        self.assertIn('"Record" { Join-Path $PSScriptRoot "agent4-physical-read-record.ps1" }', wrapper)
        self.assertIn('"Finalize" { Join-Path $PSScriptRoot "agent4-physical-read-finalize.ps1" }', wrapper)
        self.assertIn('default { Join-Path $PSScriptRoot "agent4-physical-read-operator-core.ps1" }', wrapper)
