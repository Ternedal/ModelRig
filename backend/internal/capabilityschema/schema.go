...
func (d Descriptor) Validate() error {
	...
	expected := "none"
	// Fail-closed across both axes: a non-read action OR a public-network action
	// requires confirmation. A public read changes no local state, but still
	// transmits the exact approved request to a third party.
	if d.Access != "read" || d.Network.Mode == "public" {
		expected = "required"
	}
	if d.Confirmation.Mode != expected {
		return errors.New("confirmation mode contradicts access/network")
	}
