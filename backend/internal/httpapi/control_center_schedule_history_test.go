...
func validControlCenterScheduleHistoryPayload() string {
...
func TestControlCenterScheduleHistoryProxyStampsAndValidates(t *testing.T) {
	...
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if got := r.Header.Get("X-Request-ID"); got != "req-control-center-history" {
			t.Errorf("request id = %q", got)
		}
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(validControlCenterScheduleHistoryPayload()))
	}))
	defer upstream.Close()

	s := &server{Deps: Deps{Worker: proxy.New(upstream.URL, time.Second)}}
	req := httptest.NewRequest(http.MethodGet, "/api/v1/control-center/schedules?limit=999&spoof=1", nil)
	req.Header.Set("X-Request-ID", "req-control-center-history")
	...
	s.handleControlCenterScheduleHistory(rec, req)
	...
	if payload["production_activation"] != false {
		t.Fatalf("production_activation = %#v, want false", payload["production_activation"])
	}
}

func TestControlCenterScheduleHistoryProxyFailsClosed(t *testing.T) {
	tests := []struct {
		name   string
		status int
		body   string
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			rec := httptest.NewRecorder()
			s.handleControlCenterScheduleHistory(rec, httptest.NewRequest(http.MethodGet, "/", nil))
...
func TestControlCenterScheduleHistoryRouteRequiresBearerToken(t *testing.T) {
	...
	const token = "paired-control-center-history-token"
	if err := st.AddDevice(store.Device{
		ID:        "control-center-history-device",
		Name:      "test phone",
