# Candidate 2.0.9 — physical validation head (2.0.9)

This file marks the exact head that the 2.0.9 physical campaigns are validated against. It is deleted through the normal release process after the release is tagged.
...
- version: `2.0.9`
- branch: `agent/unified-candidate-2.0.9-r2`
...
- exact 1.58.149 → 2.0.9 source/target enforcement;
...
## Physical evidence required before promoting 2.0.9
...
2. **Stage B, only after publication:** the verified one-time updater bootstrap and the normal appliance update, reboot, supervisor restart, invalid-update handling, rollback/interruption recovery and preservation matrix that can truthfully be exercised against the published signed 2.0.9 release.

No old 1.58.150 report and no report bound to #405 or any earlier invalidated 2.0.9 head may satisfy this candidate.

## Automatic updater self-update is deferred

`2.0.9` is the first signed release intended to contain self-update support. A pre-support updater cannot retroactively contain that path and requires one manual/bootstrap replacement. The verified self-update command also installs only a release newer than its compiled version.

Therefore 2.0.9 cannot truthfully prove automatic signed-release-to-signed-release self-update from 1.58.150 or from 2.0.9 to itself. Issue #401 remains open for the genuine proof using signed 2.0.9 as source and a later signed release greater than 2.0.9 as target. #401 is not a promotion blocker for 2.0.9.
...
5. After Stage A is green, require a separate explicit release decision. Fast-forward `main` to the exact frozen candidate SHA, tag that same SHA as `v2.0.9`, and publish the complete signed release set. Any SHA change invalidates Stage A.
6. Only after the signed release exists, run Stage B against published 2.0.9 and collect the updater/lifecycle evidence.
...
9. Use signed 2.0.9 as the source for issue #401 when a later signed target exists.
